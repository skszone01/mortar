...
const LANGUAGE_DATA = {
    th: {
        ...
        dispersionRadius: "รัศมีตก (m)",
        radiusLabel: "รัศมี",
        dElevPer100mHint: "มุมยกที่ต้องเปลี่ยน เมื่อความสูงต่างกัน 100 เมตร (ไม่ใช่การกระจายของกระสุน)",
        dispersionRadiusHint: "รัศมีที่กระสุนตกกระจายรอบจุดเล็ง (จากค่าในไฟล์เกม)",
    ...
    en: {
        ...
        dispersionRadius: "Dispersion (m)",
        radiusLabel: "radius",
        dElevPer100mHint: "Elevation change per 100 m of altitude difference (this is NOT shell dispersion)",
        dispersionRadiusHint: "Radius the shells scatter around the aim point (from the game files)",
    ...
    ja: {
        ...
        dispersionRadius: "散布半径 (m)",
        radiusLabel: "半径",
        dElevPer100mHint: "高度差100 mあたりに必要な仰角の変化量（散布ではありません）",
        dispersionRadiusHint: "着弾が照準点の周囲に散らばる半径（ゲームファイルの値による）",
    ...
    zh: {
        ...
        dispersionRadius: "散布半径 (m)",
        radiusLabel: "半径",
        dElevPer100mHint: "每 100 米高差所需的仰角变化量（不是弹着散布）",
        dispersionRadiusHint: "弹着点围绕瞄准点散布的半径（取自游戏文件）",
    ...
    id: {
        ...
        dispersionRadius: "Radius Sebar (m)",
        radiusLabel: "radius",
        dElevPer100mHint: "Perubahan elevasi per 100 m beda ketinggian (bukan penyebaran peluru)",
        dispersionRadiusHint: "Radius sebaran peluru di sekitar titik bidik (dari berkas game)",
    ...
    ru: {
        ...
        dispersionRadius: "Радиус рассеив. (м)",
        radiusLabel: "радиус",
        dElevPer100mHint: "Изменение угла возвышения на 100 м перепада высот (это не рассеивание)",
        dispersionRadiusHint: "Радиус разброса снарядов вокруг точки прицеливания (из файлов игры)",
    ...
    uk: {
        ...
        dispersionRadius: "Радіус розсіюв. (м)",
        radiusLabel: "радіус",
        dElevPer100mHint: "Зміна кута підвищення на 100 м перепаду висот (це не розсіювання)",
        dispersionRadiusHint: "Радіус розкиду снарядів навколо точки прицілювання (з файлів гри)",
...
function updateLanguageDisplay() {
    ...
    if (window.mortarCalculator && window.mortarCalculator.updateShellInfo) {
        ...
        if (window.mortarCalculator.updateTableHeaderTitles) {
            window.mortarCalculator.updateTableHeaderTitles();
        }
        // การ์ด RING สร้างด้วย JS ไม่มี data-lang-key จึงต้องวาดใหม่เอง
        window.mortarCalculator.renderRingPickers();
        if (window.mortarCalculator.ringSection
            && window.mortarCalculator.ringSection.classList.contains('show')) {
            if (window.mortarCalculator.validateInputs()) {
                window.mortarCalculator.calculateAvailableRings(true);
            } else if (window.mortarCalculator._lastRingArgs) {
                // พิกัดกรอกไม่ครบแล้ว คำนวณใหม่ไม่ได้ — วาดชุดเดิมซ้ำเพื่อให้ข้อความเปลี่ยนภาษาตาม
                const a = window.mortarCalculator._lastRingArgs;
                window.mortarCalculator.displayRingResults(a.rings, a.targetDistance, a.wasAdjusted);
            }
        }
...
const BALLISTIC_DATA = {
    original: {
        ...
        M821: {
            0: [
                { range: 50, elevation: 1540, timeOfFlight: 13.2, dElev100: "61m" },
                { range: 100, elevation: 1479, timeOfFlight: 13.2, dElev100: "63m" },
                { range: 150, elevation: 1416, timeOfFlight: 13.0, dElev100: "66m" },
                { range: 200, elevation: 1350, timeOfFlight: 12.8, dElev100: "71m" },
                { range: 250, elevation: 1279, timeOfFlight: 12.6, dElev100: "78m" },
                { range: 300, elevation: 1201, timeOfFlight: 12.3, dElev100: "95m" },
                { range: 350, elevation: 1106, timeOfFlight: 11.7, dElev100: "151m" },
                { range: 400, elevation: 955, timeOfFlight: 10.7, dElev100: "0m" }
            ],
            1: [
                { range: 200, elevation: 1492, timeOfFlight: 19.9, dElev100: "27m" },
                { range: 300, elevation: 1437, timeOfFlight: 19.7, dElev100: "29m" },
                { range: 400, elevation: 1378, timeOfFlight: 19.5, dElev100: "31m" },
                { range: 500, elevation: 1317, timeOfFlight: 19.2, dElev100: "33m" },
                { range: 600, elevation: 1249, timeOfFlight: 18.8, dElev100: "35m" },
                { range: 700, elevation: 1174, timeOfFlight: 18.3, dElev100: "42m" },
                { range: 800, elevation: 1085, timeOfFlight: 17.5, dElev100: "57m" },
                { range: 900, elevation: 954, timeOfFlight: 16.1, dElev100: "148m" }
            ],
            2: [
                { range: 200, elevation: 1538, timeOfFlight: 26.6, dElev100: "15m" },
                { range: 300, elevation: 1507, timeOfFlight: 26.5, dElev100: "16m" },
                { range: 400, elevation: 1475, timeOfFlight: 26.4, dElev100: "16m" },
                { range: 500, elevation: 1443, timeOfFlight: 26.3, dElev100: "16m" },
                { range: 600, elevation: 1410, timeOfFlight: 26.2, dElev100: "16m" },
                { range: 700, elevation: 1376, timeOfFlight: 26.0, dElev100: "17m" },
                { range: 800, elevation: 1341, timeOfFlight: 25.8, dElev100: "18m" },
                { range: 900, elevation: 1305, timeOfFlight: 25.5, dElev100: "20m" },
                { range: 1000, elevation: 1266, timeOfFlight: 25.2, dElev100: "20m" },
                { range: 1100, elevation: 1225, timeOfFlight: 24.9, dElev100: "22m" },
                { range: 1200, elevation: 1180, timeOfFlight: 24.4, dElev100: "23m" },
                { range: 1300, elevation: 1132, timeOfFlight: 23.9, dElev100: "27m" },
                { range: 1400, elevation: 1076, timeOfFlight: 23.2, dElev100: "31m" },
                { range: 1500, elevation: 1009, timeOfFlight: 22.3, dElev100: "43m" },
                { range: 1600, elevation: 912, timeOfFlight: 20.9, dElev100: "109m" }
            ],
            3: [
                { range: 300, elevation: 1534, timeOfFlight: 31.7, dElev100: "11m" },
                { range: 400, elevation: 1511, timeOfFlight: 31.6, dElev100: "11m" },
                { range: 500, elevation: 1489, timeOfFlight: 31.6, dElev100: "12m" },
                { range: 600, elevation: 1466, timeOfFlight: 31.5, dElev100: "12m" },
                { range: 700, elevation: 1442, timeOfFlight: 31.4, dElev100: "12m" },
                { range: 800, elevation: 1419, timeOfFlight: 31.3, dElev100: "12m" },
                { range: 900, elevation: 1395, timeOfFlight: 31.1, dElev100: "13m" },
                { range: 1000, elevation: 1370, timeOfFlight: 31.0, dElev100: "13m" },
                { range: 1100, elevation: 1344, timeOfFlight: 30.8, dElev100: "13m" },
                { range: 1200, elevation: 1318, timeOfFlight: 30.6, dElev100: "13m" },
                { range: 1300, elevation: 1291, timeOfFlight: 30.3, dElev100: "14m" },
                { range: 1400, elevation: 1263, timeOfFlight: 30.1, dElev100: "15m" },
                // แก้เวลาบิน: เดิม 29.0 ทำให้แถวถัดไป (1600 m) เป็น 29.4 คือเพิ่มขึ้นทั้งที่ยิงไกลกว่า
                { range: 1500, elevation: 1233, timeOfFlight: 29.8, dElev100: "15m" },
                { range: 1600, elevation: 1202, timeOfFlight: 29.4, dElev100: "16m" },
                { range: 1700, elevation: 1169, timeOfFlight: 29.0, dElev100: "17m" },
                { range: 1800, elevation: 1136, timeOfFlight: 28.5, dElev100: "19m" },
                { range: 1900, elevation: 1094, timeOfFlight: 28.0, dElev100: "21m" },
                { range: 2000, elevation: 1051, timeOfFlight: 27.3, dElev100: "26m" },
                { range: 2100, elevation: 999, timeOfFlight: 26.5, dElev100: "31m" },
                { range: 2200, elevation: 931, timeOfFlight: 25.3, dElev100: "46m" },
                { range: 2300, elevation: 801, timeOfFlight: 22.7, dElev100: "0m" }
            ],
            4: [
                { range: 400, elevation: 1531, timeOfFlight: 36.3, dElev100: "9m" },
                { range: 500, elevation: 1514, timeOfFlight: 36.2, dElev100: "9m" },
                { range: 600, elevation: 1496, timeOfFlight: 36.2, dElev100: "9m" },
                { range: 700, elevation: 1478, timeOfFlight: 36.1, dElev100: "9m" },
                { range: 800, elevation: 1460, timeOfFlight: 36.0, dElev100: "9m" },
                { range: 900, elevation: 1442, timeOfFlight: 35.9, dElev100: "9m" },
                { range: 1000, elevation: 1424, timeOfFlight: 35.8, dElev100: "10m" },
                { range: 1100, elevation: 1405, timeOfFlight: 35.7, dElev100: "10m" },
                { range: 1200, elevation: 1385, timeOfFlight: 35.6, dElev100: "9m" },
                { range: 1300, elevation: 1366, timeOfFlight: 35.4, dElev100: "10m" },
                { range: 1400, elevation: 1346, timeOfFlight: 35.3, dElev100: "10m" },
                { range: 1500, elevation: 1326, timeOfFlight: 35.1, dElev100: "11m" },
                { range: 1600, elevation: 1305, timeOfFlight: 34.9, dElev100: "11m" },
                { range: 1700, elevation: 1283, timeOfFlight: 34.6, dElev100: "11m" },
                { range: 1800, elevation: 1261, timeOfFlight: 34.4, dElev100: "11m" },
                { range: 1900, elevation: 1238, timeOfFlight: 34.1, dElev100: "12m" },
                { range: 2000, elevation: 1214, timeOfFlight: 33.8, dElev100: "12m" },
                { range: 2100, elevation: 1188, timeOfFlight: 33.5, dElev100: "13m" },
                { range: 2200, elevation: 1162, timeOfFlight: 33.1, dElev100: "14m" },
                { range: 2300, elevation: 1134, timeOfFlight: 32.7, dElev100: "15m" },
                { range: 2400, elevation: 1104, timeOfFlight: 32.2, dElev100: "17m" },
                { range: 2500, elevation: 1070, timeOfFlight: 31.7, dElev100: "17m" },
                { range: 2600, elevation: 1034, timeOfFlight: 31.0, dElev100: "20m" },
                { range: 2700, elevation: 993, timeOfFlight: 30.3, dElev100: "25m" },
                { range: 2800, elevation: 942, timeOfFlight: 29.2, dElev100: "31m" },
                { range: 2900, elevation: 870, timeOfFlight: 27.7, dElev100: "64m" }
            ]
        },
        M819: {
            1: [
                { range: 200, elevation: 1463, timeOfFlight: 17.7, dElev100: "36m" },
                { range: 250, elevation: 1427, timeOfFlight: 17.6, dElev100: "36m" },
                { range: 300, elevation: 1391, timeOfFlight: 17.5, dElev100: "39m" },
                { range: 350, elevation: 1352, timeOfFlight: 17.3, dElev100: "38m" },
                { range: 400, elevation: 1314, timeOfFlight: 17.2, dElev100: "43m" },
                { range: 450, elevation: 1271, timeOfFlight: 16.9, dElev100: "44m" },
                { range: 500, elevation: 1227, timeOfFlight: 16.7, dElev100: "49m" },
                { range: 550, elevation: 1178, timeOfFlight: 16.4, dElev100: "54m" },
                { range: 600, elevation: 1124, timeOfFlight: 16.0, dElev100: "64m" },
                { range: 650, elevation: 1060, timeOfFlight: 15.4, dElev100: "78m" },
                { range: 700, elevation: 982, timeOfFlight: 14.7, dElev100: "160m" },
                { range: 750, elevation: 822, timeOfFlight: 13.0, dElev100: "0m" }
            ],
            2: [
                { range: 200, elevation: 1538, timeOfFlight: 24.8, dElev100: "19m" },
                { range: 300, elevation: 1491, timeOfFlight: 24.7, dElev100: "19m" },
                { range: 400, elevation: 1453, timeOfFlight: 24.6, dElev100: "19m" },
                { range: 500, elevation: 1414, timeOfFlight: 24.4, dElev100: "19m" },
                { range: 600, elevation: 1374, timeOfFlight: 24.2, dElev100: "20m" },
                { range: 700, elevation: 1333, timeOfFlight: 24.0, dElev100: "22m" },
                { range: 800, elevation: 1289, timeOfFlight: 23.7, dElev100: "23m" },
                { range: 900, elevation: 1242, timeOfFlight: 23.3, dElev100: "25m" },
                { range: 1000, elevation: 1191, timeOfFlight: 22.9, dElev100: "28m" },
                { range: 1100, elevation: 1135, timeOfFlight: 22.3, dElev100: "31m" },
                { range: 1200, elevation: 1067, timeOfFlight: 21.6, dElev100: "39m" },
                { range: 1300, elevation: 980, timeOfFlight: 20.5, dElev100: "58m" },
                { range: 1400, elevation: 818, timeOfFlight: 18.0, dElev100: "0m" }
            ],
            3: [
                { range: 300, elevation: 1522, timeOfFlight: 29.6, dElev100: "14m" },
                { range: 400, elevation: 1495, timeOfFlight: 29.6, dElev100: "14m" },
                { range: 500, elevation: 1467, timeOfFlight: 29.5, dElev100: "14m" },
                { range: 600, elevation: 1440, timeOfFlight: 29.3, dElev100: "14m" },
                { range: 700, elevation: 1412, timeOfFlight: 29.2, dElev100: "14m" },
                { range: 800, elevation: 1383, timeOfFlight: 29.0, dElev100: "14m" },
                { range: 900, elevation: 1354, timeOfFlight: 28.9, dElev100: "16m" },
                { range: 1000, elevation: 1323, timeOfFlight: 28.6, dElev100: "16m" },
                { range: 1100, elevation: 1291, timeOfFlight: 28.4, dElev100: "17m" },
                { range: 1200, elevation: 1257, timeOfFlight: 28.1, dElev100: "18m" },
                { range: 1300, elevation: 1221, timeOfFlight: 27.7, dElev100: "20m" },
                { range: 1400, elevation: 1183, timeOfFlight: 27.3, dElev100: "23m" },
                { range: 1500, elevation: 1142, timeOfFlight: 26.8, dElev100: "25m" },
                { range: 1600, elevation: 1096, timeOfFlight: 26.2, dElev100: "30m" },
                { range: 1700, elevation: 1044, timeOfFlight: 25.5, dElev100: "38m" },
                { range: 1800, elevation: 980, timeOfFlight: 24.5, dElev100: "84m" },
                { range: 1900, elevation: 892, timeOfFlight: 23.0, dElev100: "0m" }
            ],
            4: [
                { range: 400, elevation: 1517, timeOfFlight: 33.6, dElev100: "11m" },
                { range: 500, elevation: 1495, timeOfFlight: 33.5, dElev100: "10m" },
                { range: 600, elevation: 1474, timeOfFlight: 33.5, dElev100: "11m" },
                { range: 700, elevation: 1452, timeOfFlight: 33.4, dElev100: "11m" },
                { range: 800, elevation: 1429, timeOfFlight: 33.2, dElev100: "11m" },
                { range: 900, elevation: 1407, timeOfFlight: 33.1, dElev100: "12m" },
                { range: 1000, elevation: 1383, timeOfFlight: 33.0, dElev100: "11m" },
                { range: 1100, elevation: 1360, timeOfFlight: 32.8, dElev100: "12m" },
                { range: 1200, elevation: 1335, timeOfFlight: 32.6, dElev100: "12m" },
                { range: 1300, elevation: 1310, timeOfFlight: 32.4, dElev100: "13m" },
                { range: 1400, elevation: 1284, timeOfFlight: 32.1, dElev100: "14m" },
                { range: 1500, elevation: 1257, timeOfFlight: 31.9, dElev100: "14m" },
                { range: 1600, elevation: 1228, timeOfFlight: 31.6, dElev100: "15m" },
                { range: 1700, elevation: 1199, timeOfFlight: 31.2, dElev100: "17m" },
                { range: 1800, elevation: 1166, timeOfFlight: 30.8, dElev100: "16m" },
                { range: 1900, elevation: 1132, timeOfFlight: 30.3, dElev100: "18m" },
                { range: 2000, elevation: 1096, timeOfFlight: 29.8, dElev100: "21m" },
                { range: 2100, elevation: 1055, timeOfFlight: 29.1, dElev100: "23m" },
                { range: 2200, elevation: 1008, timeOfFlight: 28.4, dElev100: "28m" },
                { range: 2300, elevation: 952, timeOfFlight: 27.4, dElev100: "36m" },
                { range: 2400, elevation: 871, timeOfFlight: 25.8, dElev100: "67m" }
            ]
        },
        M853A1: {
            1: [
                { range: 200, elevation: 1463, timeOfFlight: 18.1, dElev100: "35m" },
                { range: 250, elevation: 1428, timeOfFlight: 18.0, dElev100: "37m" },
                { range: 300, elevation: 1391, timeOfFlight: 17.9, dElev100: "39m" },
                { range: 350, elevation: 1352, timeOfFlight: 17.7, dElev100: "40m" },
                { range: 400, elevation: 1312, timeOfFlight: 17.5, dElev100: "43m" },
                { range: 450, elevation: 1269, timeOfFlight: 17.3, dElev100: "45m" },
                { range: 500, elevation: 1224, timeOfFlight: 17.0, dElev100: "49m" },
                { range: 550, elevation: 1175, timeOfFlight: 16.7, dElev100: "55m" },
                { range: 600, elevation: 1120, timeOfFlight: 16.3, dElev100: "65m" },
                { range: 650, elevation: 1055, timeOfFlight: 15.7, dElev100: "81m" },
                { range: 700, elevation: 974, timeOfFlight: 15.0, dElev100: "151m" },
                { range: 750, elevation: 823, timeOfFlight: 13.3, dElev100: "0m" }
            ],
            2: [
                { range: 200, elevation: 1529, timeOfFlight: 26.2, dElev100: "17m" },
                { range: 300, elevation: 1496, timeOfFlight: 26.1, dElev100: "18m" },
                { range: 400, elevation: 1457, timeOfFlight: 26.0, dElev100: "19m" },
                { range: 500, elevation: 1419, timeOfFlight: 25.8, dElev100: "19m" },
                { range: 600, elevation: 1379, timeOfFlight: 25.6, dElev100: "20m" },
                { range: 700, elevation: 1338, timeOfFlight: 25.4, dElev100: "21m" },
                { range: 800, elevation: 1295, timeOfFlight: 25.1, dElev100: "23m" },
                { range: 900, elevation: 1249, timeOfFlight: 24.7, dElev100: "25m" },
                { range: 1000, elevation: 1199, timeOfFlight: 24.3, dElev100: "27m" },
                { range: 1100, elevation: 1144, timeOfFlight: 23.7, dElev100: "30m" },
                { range: 1200, elevation: 1081, timeOfFlight: 23.0, dElev100: "35m" },
                { range: 1300, elevation: 1005, timeOfFlight: 22.0, dElev100: "47m" },
                { range: 1400, elevation: 900, timeOfFlight: 20.5, dElev100: "98m" }
            ],
            3: [
                { range: 300, elevation: 1521, timeOfFlight: 31.1, dElev100: "14m" },
                { range: 400, elevation: 1494, timeOfFlight: 31.1, dElev100: "14m" },
                { range: 500, elevation: 1466, timeOfFlight: 31.0, dElev100: "14m" },
                { range: 600, elevation: 1438, timeOfFlight: 30.8, dElev100: "14m" },
                { range: 700, elevation: 1409, timeOfFlight: 30.7, dElev100: "14m" },
                { range: 800, elevation: 1380, timeOfFlight: 30.5, dElev100: "16m" },
                { range: 900, elevation: 1349, timeOfFlight: 30.3, dElev100: "16m" },
                { range: 1000, elevation: 1317, timeOfFlight: 30.1, dElev100: "16m" },
                { range: 1100, elevation: 1284, timeOfFlight: 29.8, dElev100: "18m" },
                { range: 1200, elevation: 1249, timeOfFlight: 29.4, dElev100: "19m" },
                { range: 1300, elevation: 1212, timeOfFlight: 29.1, dElev100: "20m" },
                { range: 1400, elevation: 1172, timeOfFlight: 28.6, dElev100: "21m" },
                { range: 1500, elevation: 1128, timeOfFlight: 28.1, dElev100: "22m" },
                { range: 1600, elevation: 1081, timeOfFlight: 27.4, dElev100: "26m" },
                { range: 1700, elevation: 1027, timeOfFlight: 26.6, dElev100: "30m" },
                { range: 1800, elevation: 962, timeOfFlight: 25.6, dElev100: "39m" },
                { range: 1900, elevation: 875, timeOfFlight: 24.1, dElev100: "67m" }
            ],
            4: [
                { range: 400, elevation: 1515, timeOfFlight: 35.7, dElev100: "11m" },
                { range: 500, elevation: 1493, timeOfFlight: 35.7, dElev100: "11m" },
                { range: 600, elevation: 1471, timeOfFlight: 35.6, dElev100: "11m" },
                { range: 700, elevation: 1448, timeOfFlight: 35.5, dElev100: "11m" },
                { range: 800, elevation: 1426, timeOfFlight: 35.4, dElev100: "12m" },
                { range: 900, elevation: 1402, timeOfFlight: 35.2, dElev100: "12m" },
                { range: 1000, elevation: 1378, timeOfFlight: 35.0, dElev100: "12m" },
                { range: 1100, elevation: 1353, timeOfFlight: 34.8, dElev100: "13m" },
                { range: 1200, elevation: 1328, timeOfFlight: 34.6, dElev100: "13m" },
                { range: 1300, elevation: 1301, timeOfFlight: 34.4, dElev100: "14m" },
                { range: 1400, elevation: 1274, timeOfFlight: 34.1, dElev100: "14m" },
                { range: 1500, elevation: 1245, timeOfFlight: 33.8, dElev100: "15m" },
                { range: 1600, elevation: 1215, timeOfFlight: 33.4, dElev100: "15m" },
                { range: 1700, elevation: 1184, timeOfFlight: 33.0, dElev100: "17m" },
                { range: 1800, elevation: 1151, timeOfFlight: 32.6, dElev100: "18m" },
                { range: 1900, elevation: 1115, timeOfFlight: 32.1, dElev100: "19m" },
                { range: 2000, elevation: 1076, timeOfFlight: 31.5, dElev100: "21m" },
                { range: 2100, elevation: 1033, timeOfFlight: 30.8, dElev100: "23m" },
                { range: 2200, elevation: 985, timeOfFlight: 29.9, dElev100: "27m" },
                { range: 2300, elevation: 928, timeOfFlight: 28.8, dElev100: "33m" },
                { range: 2400, elevation: 855, timeOfFlight: 27.4, dElev100: "52m" }
            ]
        },
        ...
        "0-832Ay": {
            ...
            0: [
                { range: 50, elevation: 1455, timeOfFlight: 15, dElev100: "44m" },
                { range: 100, elevation: 1411, timeOfFlight: 15, dElev100: "46m" },
                { range: 150, elevation: 1365, timeOfFlight: 14.9, dElev100: "47m" },
                { range: 200, elevation: 1318, timeOfFlight: 14.8, dElev100: "50m" },
                { range: 250, elevation: 1268, timeOfFlight: 14.6, dElev100: "51m" },
                { range: 300, elevation: 1217, timeOfFlight: 14.4, dElev100: "58m" },
                { range: 350, elevation: 1159, timeOfFlight: 14.1, dElev100: "64m" },
                { range: 400, elevation: 1095, timeOfFlight: 13.7, dElev100: "72m" },
                { range: 450, elevation: 1023, timeOfFlight: 13.2, dElev100: "101m" },
                { range: 500, elevation: 922, timeOfFlight: 12.4, dElev100: "0m" }
            ],
            // 1 Ring - Original (Dispersion: 13m ตามภาพที่ 1)
            1: [
                { range: 100, elevation: 1446, timeOfFlight: 19.5, dElev100: "27m" },
                { range: 200, elevation: 1392, timeOfFlight: 19.4, dElev100: "28m" },
                { range: 300, elevation: 1335, timeOfFlight: 19.2, dElev100: "29m" },
                { range: 400, elevation: 1275, timeOfFlight: 18.9, dElev100: "31m" },
                { range: 500, elevation: 1212, timeOfFlight: 18.6, dElev100: "35m" },
                { range: 600, elevation: 1141, timeOfFlight: 18.1, dElev100: "40m" },
                { range: 700, elevation: 1058, timeOfFlight: 17.4, dElev100: "48m" },
                { range: 800, elevation: 952, timeOfFlight: 16.4, dElev100: "81m" }
            ],
            // 2 Ring - Original (Dispersion: 19m ตามภาพที่ 1)
            2: [
                { range: 200, elevation: 1432, timeOfFlight: 24.8, dElev100: "17m" },
                { range: 300, elevation: 1397, timeOfFlight: 24.7, dElev100: "18m" },
                { range: 400, elevation: 1362, timeOfFlight: 24.6, dElev100: "18m" },
                { range: 500, elevation: 1325, timeOfFlight: 24.4, dElev100: "18m" },
                { range: 600, elevation: 1288, timeOfFlight: 24.2, dElev100: "20m" },
                { range: 700, elevation: 1248, timeOfFlight: 24, dElev100: "20m" },
                { range: 800, elevation: 1207, timeOfFlight: 23.7, dElev100: "22m" },
                { range: 900, elevation: 1162, timeOfFlight: 23.3, dElev100: "23m" },
                { range: 1000, elevation: 1114, timeOfFlight: 22.9, dElev100: "26m" },
                { range: 1100, elevation: 1060, timeOfFlight: 22.3, dElev100: "29m" },
                { range: 1200, elevation: 997, timeOfFlight: 21.5, dElev100: "37m" },
                { range: 1300, elevation: 914, timeOfFlight: 20.4, dElev100: "55m" },
                { range: 1400, elevation: 755, timeOfFlight: 17.8, dElev100: "0m" }
            ],
            // 3 Ring - Original (Dispersion: 27m ตามภาพที่ 1)
            3: [
                { range: 300, elevation: 1423, timeOfFlight: 28.9, dElev100: "13m" },
                { range: 400, elevation: 1397, timeOfFlight: 28.8, dElev100: "14m" },
                { range: 500, elevation: 1370, timeOfFlight: 28.6, dElev100: "13m" },
                { range: 600, elevation: 1343, timeOfFlight: 28.5, dElev100: "14m" },
                { range: 700, elevation: 1315, timeOfFlight: 28.5, dElev100: "14m" },
                { range: 800, elevation: 1286, timeOfFlight: 28.3, dElev100: "14m" },
                { range: 900, elevation: 1257, timeOfFlight: 28.1, dElev100: "16m" },
                { range: 1000, elevation: 1226, timeOfFlight: 27.9, dElev100: "16m" },
                { range: 1100, elevation: 1193, timeOfFlight: 27.6, dElev100: "16m" },
                { range: 1200, elevation: 1159, timeOfFlight: 27.2, dElev100: "18m" },
                { range: 1300, elevation: 1123, timeOfFlight: 26.8, dElev100: "19m" },
                { range: 1400, elevation: 1084, timeOfFlight: 26.4, dElev100: "22m" },
                { range: 1500, elevation: 1040, timeOfFlight: 25.8, dElev100: "24m" },
                { range: 1600, elevation: 991, timeOfFlight: 25.1, dElev100: "28m" },
                { range: 1700, elevation: 932, timeOfFlight: 24.2, dElev100: "36m" },
                { range: 1800, elevation: 851, timeOfFlight: 22.8, dElev100: "68m" }
            ],
            // 4 Ring - Original (Dispersion: 34m ตามภาพที่ 1)
            4: [
                { range: 400, elevation: 1418, timeOfFlight: 32.9, dElev100: "10m" },
                { range: 500, elevation: 1398, timeOfFlight: 32.9, dElev100: "11m" },
                { range: 600, elevation: 1376, timeOfFlight: 32.8, dElev100: "10m" },
                { range: 700, elevation: 1355, timeOfFlight: 32.7, dElev100: "11m" },
                { range: 800, elevation: 1333, timeOfFlight: 32.6, dElev100: "11m" },
                { range: 900, elevation: 1311, timeOfFlight: 32.4, dElev100: "12m" },
                { range: 1000, elevation: 1288, timeOfFlight: 32.2, dElev100: "12m" },
                { range: 1100, elevation: 1264, timeOfFlight: 32.1, dElev100: "12m" },
                { range: 1200, elevation: 1240, timeOfFlight: 31.8, dElev100: "13m" },
                { range: 1300, elevation: 1215, timeOfFlight: 31.6, dElev100: "13m" },
                { range: 1400, elevation: 1189, timeOfFlight: 31.3, dElev100: "14m" },
                { range: 1500, elevation: 1161, timeOfFlight: 31, dElev100: "14m" },
                { range: 1600, elevation: 1133, timeOfFlight: 30.7, dElev100: "15m" },
                { range: 1700, elevation: 1102, timeOfFlight: 30.3, dElev100: "16m" },
                { range: 1800, elevation: 1069, timeOfFlight: 29.8, dElev100: "17m" },
                { range: 1900, elevation: 1034, timeOfFlight: 29.3, dElev100: "19m" },
                { range: 2000, elevation: 995, timeOfFlight: 28.7, dElev100: "22m" },
                { range: 2100, elevation: 950, timeOfFlight: 27.9, dElev100: "26m" },
                { range: 2200, elevation: 896, timeOfFlight: 26.9, dElev100: "34m" },
                { range: 2300, elevation: 820, timeOfFlight: 25.3, dElev100: "65m" }
            ]
        },
        "A-832AY": {
            // 0 Ring - Original (Dispersion: 7m ตามภาพที่ 1)
            0: [
                { range: 50, elevation: 1450, timeOfFlight: 14.1, dElev100: "51m" },
                { range: 100, elevation: 1399, timeOfFlight: 14, dElev100: "52m" },
                { range: 150, elevation: 1347, timeOfFlight: 13.9, dElev100: "55m" },
                { range: 200, elevation: 1292, timeOfFlight: 13.8, dElev100: "57m" },
                { range: 250, elevation: 1235, timeOfFlight: 13.6, dElev100: "63m" },
                { range: 300, elevation: 1172, timeOfFlight: 13.3, dElev100: "70m" },
                { range: 350, elevation: 1102, timeOfFlight: 12.9, dElev100: "82m" },
                { range: 400, elevation: 1020, timeOfFlight: 12.4, dElev100: "122m" },
                { range: 450, elevation: 898, timeOfFlight: 11.4, dElev100: "0m" }
            ],
            // 1 Ring - Original (Dispersion: 12m ตามภาพที่ 2)  
            1: [
                { range: 200, elevation: 1381, timeOfFlight: 18.4, dElev100: "31m" },
                { range: 300, elevation: 1319, timeOfFlight: 18.2, dElev100: "33m" },
                { range: 400, elevation: 1252, timeOfFlight: 17.9, dElev100: "34m" },
                { range: 500, elevation: 1179, timeOfFlight: 17.5, dElev100: "38m" },
                { range: 600, elevation: 1097, timeOfFlight: 16.9, dElev100: "47m" },
                { range: 700, elevation: 993, timeOfFlight: 16.0, dElev100: "67m" },
                { range: 800, elevation: 805, timeOfFlight: 13.9, dElev100: "0m" }
            ],
            // 2 Ring - Original (Dispersion: 18m ตามภาพที่ 3)
            2: [
                { range: 300, elevation: 1387, timeOfFlight: 23.5, dElev100: "19m" },
                // แก้เวลาบิน: เดิม 23.8 มากกว่าแถวก่อนหน้า (300 m = 23.5) ทั้งที่ยิงไกลกว่า
                { range: 400, elevation: 1348, timeOfFlight: 23.4, dElev100: "20m" },
                { range: 500, elevation: 1308, timeOfFlight: 23.2, dElev100: "21m" },
                { range: 600, elevation: 1266, timeOfFlight: 22.9, dElev100: "22m" },
                { range: 700, elevation: 1222, timeOfFlight: 22.7, dElev100: "24m" },
                { range: 800, elevation: 1175, timeOfFlight: 22.3, dElev100: "26m" },
                { range: 900, elevation: 1123, timeOfFlight: 21.8, dElev100: "28m" },
                { range: 1000, elevation: 1065, timeOfFlight: 21.3, dElev100: "32m" },
                { range: 1100, elevation: 994, timeOfFlight: 20.4, dElev100: "40m" },
                { range: 1200, elevation: 902, timeOfFlight: 19.2, dElev100: "64m" }
            ],
            // 3 Ring - Original (Dispersion: 24m ตามภาพที่ 4)
            3: [
                { range: 400, elevation: 1387, timeOfFlight: 27.3, dElev100: "15m" },
                { range: 500, elevation: 1357, timeOfFlight: 27.2, dElev100: "15m" },
                { range: 600, elevation: 1327, timeOfFlight: 27.1, dElev100: "15m" },
                { range: 700, elevation: 1296, timeOfFlight: 26.9, dElev100: "16m" },
                { range: 800, elevation: 1264, timeOfFlight: 26.7, dElev100: "16m" },
                { range: 900, elevation: 1231, timeOfFlight: 26.5, dElev100: "17m" },
                { range: 1000, elevation: 1196, timeOfFlight: 26.2, dElev100: "18m" },
                { range: 1100, elevation: 1159, timeOfFlight: 25.8, dElev100: "20m" },
                { range: 1200, elevation: 1119, timeOfFlight: 25.4, dElev100: "22m" },
                { range: 1300, elevation: 1075, timeOfFlight: 24.9, dElev100: "24m" },
                { range: 1400, elevation: 1026, timeOfFlight: 24.3, dElev100: "27m" },
                { range: 1500, elevation: 969, timeOfFlight: 23.5, dElev100: "33m" },
                { range: 1600, elevation: 896, timeOfFlight: 22.3, dElev100: "50m" },
                { range: 1700, elevation: 753, timeOfFlight: 19.8, dElev100: "0m" }
            ]
        },
        ...
        "C-832C": {
            ...
            1: [
                ...
                { range: 100, elevation: 1421, timeOfFlight: 16.5, dElev100: "40m" },
                { range: 150, elevation: 1381, timeOfFlight: 16.3, dElev100: "42m" },
                { range: 200, elevation: 1339, timeOfFlight: 16.2, dElev100: "43m" },
                { range: 250, elevation: 1296, timeOfFlight: 16.1, dElev100: "46m" },
                { range: 300, elevation: 1251, timeOfFlight: 15.9, dElev100: "48m" },
                { range: 350, elevation: 1203, timeOfFlight: 15.7, dElev100: "52m" },
                { range: 400, elevation: 1151, timeOfFlight: 15.4, dElev100: "58m" },
                { range: 450, elevation: 1093, timeOfFlight: 15, dElev100: "65m" },
                { range: 500, elevation: 1028, timeOfFlight: 14.5, dElev100: "83m" },
                { range: 550, elevation: 945, timeOfFlight: 13.8, dElev100: "146m" },
                { range: 600, elevation: 799, timeOfFlight: 12.3, dElev100: "0m" }
            ],
            // 2 Ring - Original C-832C Illumination (ตามภาพ Ring 2 - Dispersion: 17m)
            2: [
                { range: 200, elevation: 1417, timeOfFlight: 23.6, dElev100: "21m" },
                { range: 300, elevation: 1374, timeOfFlight: 23.5, dElev100: "22m" },
                { range: 400, elevation: 1330, timeOfFlight: 23.3, dElev100: "23m" },
                { range: 500, elevation: 1284, timeOfFlight: 23.1, dElev100: "24m" },
                { range: 600, elevation: 1234, timeOfFlight: 22.8, dElev100: "25m" },
                { range: 700, elevation: 1182, timeOfFlight: 22.4, dElev100: "29m" },
                { range: 800, elevation: 1124, timeOfFlight: 21.9, dElev100: "32m" },
                { range: 900, elevation: 1057, timeOfFlight: 21.3, dElev100: "36m" },
                { range: 1000, elevation: 979, timeOfFlight: 20.4, dElev100: "48m" },
                { range: 1100, elevation: 870, timeOfFlight: 18.9, dElev100: "89m" }
            ],
            // 3 Ring - Original C-832C Illumination (ตามภาพ Ring 3 - Dispersion: 24m)
            3: [
                { range: 300, elevation: 1411, timeOfFlight: 29, dElev100: "16m" },
                { range: 400, elevation: 1380, timeOfFlight: 28.9, dElev100: "16m" },
                { range: 500, elevation: 1348, timeOfFlight: 28.7, dElev100: "16m" },
                { range: 600, elevation: 1315, timeOfFlight: 28.6, dElev100: "16m" },
                { range: 700, elevation: 1281, timeOfFlight: 28.4, dElev100: "17m" },
                { range: 800, elevation: 1246, timeOfFlight: 28.1, dElev100: "18m" },
                { range: 900, elevation: 1209, timeOfFlight: 27.8, dElev100: "19m" },
                { range: 1000, elevation: 1170, timeOfFlight: 27.4, dElev100: "21m" },
                { range: 1100, elevation: 1128, timeOfFlight: 27, dElev100: "23m" },
                { range: 1200, elevation: 1082, timeOfFlight: 26.5, dElev100: "25m" },
                { range: 1300, elevation: 1031, timeOfFlight: 25.8, dElev100: "28m" },
                { range: 1400, elevation: 973, timeOfFlight: 25, dElev100: "33m" },
                { range: 1500, elevation: 903, timeOfFlight: 23.9, dElev100: "43m" },
                { range: 1600, elevation: 807, timeOfFlight: 22.3, dElev100: "0m" }
            ],
            // 4 Ring - Original C-832C Illumination (ตามภาพ Ring 4 - Dispersion: 32m)
            4: [
                { range: 400, elevation: 1411, timeOfFlight: 35.3, dElev100: "12m" },
                { range: 500, elevation: 1388, timeOfFlight: 35.2, dElev100: "12m" },
                { range: 600, elevation: 1364, timeOfFlight: 35.1, dElev100: "11m" },
                { range: 700, elevation: 1341, timeOfFlight: 35, dElev100: "13m" },
                { range: 800, elevation: 1316, timeOfFlight: 34.8, dElev100: "13m" },
                { range: 900, elevation: 1291, timeOfFlight: 34.7, dElev100: "13m" },
                { range: 1000, elevation: 1265, timeOfFlight: 34.4, dElev100: "13m" },
                { range: 1100, elevation: 1238, timeOfFlight: 34.2, dElev100: "14m" },
                { range: 1200, elevation: 1210, timeOfFlight: 33.9, dElev100: "14m" },
                { range: 1300, elevation: 1181, timeOfFlight: 33.6, dElev100: "15m" },
                { range: 1400, elevation: 1150, timeOfFlight: 33.2, dElev100: "15m" },
                { range: 1500, elevation: 1119, timeOfFlight: 32.8, dElev100: "17m" },
                { range: 1600, elevation: 1085, timeOfFlight: 32.4, dElev100: "18m" },
                { range: 1700, elevation: 1048, timeOfFlight: 31.8, dElev100: "19m" },
                { range: 1800, elevation: 1009, timeOfFlight: 31.2, dElev100: "21m" },
                { range: 1900, elevation: 965, timeOfFlight: 30.4, dElev100: "23m" },
                { range: 2000, elevation: 917, timeOfFlight: 29.6, dElev100: "27m" },
                { range: 2100, elevation: 860, timeOfFlight: 28.4, dElev100: "34m" },
                { range: 2200, elevation: 787, timeOfFlight: 26.9, dElev100: "0m" }
            ]
        }
    },
    mod: {
        M821: {
            ...
            1: [
                { range: 300, elevation: 1520, timeOfFlight: 28.6, dElev100: "14m" },
                { range: 400, elevation: 1492, timeOfFlight: 28.6, dElev100: "13m" },
                { range: 500, elevation: 1465, timeOfFlight: 28.5, dElev100: "14m" },
                { range: 600, elevation: 1437, timeOfFlight: 28.3, dElev100: "15m" },
                { range: 700, elevation: 1408, timeOfFlight: 28.2, dElev100: "15m" },
                { range: 800, elevation: 1378, timeOfFlight: 28.0, dElev100: "15m" },
                { range: 900, elevation: 1348, timeOfFlight: 27.9, dElev100: "16m" },
                { range: 1000, elevation: 1316, timeOfFlight: 27.6, dElev100: "15m" },
                { range: 1100, elevation: 1284, timeOfFlight: 27.5, dElev100: "17m" },
                { range: 1200, elevation: 1249, timeOfFlight: 27.1, dElev100: "18m" },
                { range: 1300, elevation: 1213, timeOfFlight: 26.7, dElev100: "20m" },
                { range: 1400, elevation: 1173, timeOfFlight: 26.3, dElev100: "20m" },
                { range: 1500, elevation: 1130, timeOfFlight: 25.7, dElev100: "23m" },
                { range: 1600, elevation: 1082, timeOfFlight: 25.1, dElev100: "26m" },
                { range: 1700, elevation: 1026, timeOfFlight: 24.3, dElev100: "33m" },
                { range: 1800, elevation: 955, timeOfFlight: 23.2, dElev100: "50m" },
                { range: 1900, elevation: 841, timeOfFlight: 20.7, dElev100: "0m" }
            ],
            // Charge 2 - MOD Adult Mortars (2 Rings) - Average Dispersion: 39m
            2: [
                { range: 500, elevation: 1523, timeOfFlight: 38.6, dElev100: "8m" },
                { range: 600, elevation: 1507, timeOfFlight: 38.5, dElev100: "8m" },
                { range: 700, elevation: 1491, timeOfFlight: 38.5, dElev100: "8m" },
                { range: 800, elevation: 1475, timeOfFlight: 38.4, dElev100: "8m" },
                { range: 900, elevation: 1459, timeOfFlight: 38.3, dElev100: "8m" },
                { range: 1000, elevation: 1443, timeOfFlight: 38.2, dElev100: "8m" },
                { range: 1100, elevation: 1426, timeOfFlight: 38.1, dElev100: "8m" },
                { range: 1200, elevation: 1410, timeOfFlight: 38.0, dElev100: "9m" },
                { range: 1300, elevation: 1393, timeOfFlight: 37.9, dElev100: "9m" },
                { range: 1400, elevation: 1375, timeOfFlight: 37.7, dElev100: "9m" },
                { range: 1500, elevation: 1358, timeOfFlight: 37.6, dElev100: "9m" },
                { range: 1600, elevation: 1339, timeOfFlight: 37.4, dElev100: "9m" },
                { range: 1700, elevation: 1321, timeOfFlight: 37.3, dElev100: "9m" },
                { range: 1800, elevation: 1302, timeOfFlight: 37.1, dElev100: "10m" },
                { range: 1900, elevation: 1282, timeOfFlight: 36.8, dElev100: "10m" },
                { range: 2000, elevation: 1263, timeOfFlight: 36.6, dElev100: "10m" },
                { range: 2100, elevation: 1242, timeOfFlight: 36.3, dElev100: "11m" },
                { range: 2200, elevation: 1220, timeOfFlight: 36.1, dElev100: "10m" },
                { range: 2300, elevation: 1199, timeOfFlight: 35.8, dElev100: "12m" },
                { range: 2400, elevation: 1175, timeOfFlight: 35.4, dElev100: "12m" },
                { range: 2500, elevation: 1151, timeOfFlight: 35.1, dElev100: "13m" },
                { range: 2600, elevation: 1125, timeOfFlight: 34.6, dElev100: "14m" },
                { range: 2700, elevation: 1098, timeOfFlight: 34.2, dElev100: "16m" },
                { range: 2800, elevation: 1067, timeOfFlight: 33.6, dElev100: "15m" },
                { range: 2900, elevation: 1035, timeOfFlight: 33.0, dElev100: "18m" },
                { range: 3000, elevation: 999, timeOfFlight: 32.3, dElev100: "22m" },
                { range: 3100, elevation: 955, timeOfFlight: 31.4, dElev100: "26m" },
                { range: 3200, elevation: 901, timeOfFlight: 30.2, dElev100: "39m" },
                { range: 3300, elevation: 804, timeOfFlight: 27.8, dElev100: "0m" }
            ],
            // Charge 3 - MOD Adult Mortars (3 Rings) - Average Dispersion: 54m
            3: [
                { range: 700, elevation: 1522, timeOfFlight: 46.5, dElev100: "6m" },
                { range: 800, elevation: 1511, timeOfFlight: 46.4, dElev100: "6m" },
                { range: 900, elevation: 1499, timeOfFlight: 46.4, dElev100: "5m" },
                { range: 1000, elevation: 1488, timeOfFlight: 46.3, dElev100: "6m" },
                { range: 1100, elevation: 1476, timeOfFlight: 46.3, dElev100: "5m" },
                { range: 1200, elevation: 1465, timeOfFlight: 46.2, dElev100: "6m" },
                { range: 1300, elevation: 1453, timeOfFlight: 46.1, dElev100: "6m" },
                { range: 1400, elevation: 1441, timeOfFlight: 46.0, dElev100: "6m" },
                { range: 1500, elevation: 1429, timeOfFlight: 45.9, dElev100: "6m" },
                { range: 1600, elevation: 1417, timeOfFlight: 45.9, dElev100: "6m" },
                { range: 1700, elevation: 1405, timeOfFlight: 45.8, dElev100: "6m" },
                { range: 1800, elevation: 1393, timeOfFlight: 45.6, dElev100: "7m" },
                { range: 1900, elevation: 1380, timeOfFlight: 45.5, dElev100: "6m" },
                { range: 2000, elevation: 1367, timeOfFlight: 45.4, dElev100: "6m" },
                { range: 2100, elevation: 1354, timeOfFlight: 45.3, dElev100: "6m" },
                { range: 2200, elevation: 1341, timeOfFlight: 45.1, dElev100: "6m" },
                { range: 2300, elevation: 1328, timeOfFlight: 45.0, dElev100: "7m" },
                { range: 2400, elevation: 1314, timeOfFlight: 44.8, dElev100: "6m" },
                { range: 2500, elevation: 1301, timeOfFlight: 44.6, dElev100: "7m" },
                { range: 2600, elevation: 1287, timeOfFlight: 44.4, dElev100: "7m" },
                { range: 2700, elevation: 1272, timeOfFlight: 44.3, dElev100: "7m" },
                { range: 2800, elevation: 1258, timeOfFlight: 44.0, dElev100: "8m" },
                { range: 2900, elevation: 1243, timeOfFlight: 43.8, dElev100: "8m" },
                { range: 3000, elevation: 1227, timeOfFlight: 43.6, dElev100: "8m" },
                { range: 3100, elevation: 1211, timeOfFlight: 43.3, dElev100: "8m" },
                { range: 3200, elevation: 1195, timeOfFlight: 43.0, dElev100: "8m" },
                { range: 3300, elevation: 1178, timeOfFlight: 42.8, dElev100: "9m" },
                { range: 3400, elevation: 1161, timeOfFlight: 42.4, dElev100: "9m" },
                { range: 3500, elevation: 1142, timeOfFlight: 42.1, dElev100: "9m" },
                { range: 3600, elevation: 1124, timeOfFlight: 41.7, dElev100: "10m" },
                { range: 3700, elevation: 1104, timeOfFlight: 41.3, dElev100: "10m" },
                { range: 3800, elevation: 1083, timeOfFlight: 40.9, dElev100: "11m" },
                { range: 3900, elevation: 1061, timeOfFlight: 40.4, dElev100: "11m" },
                { range: 4000, elevation: 1037, timeOfFlight: 39.9, dElev100: "13m" },
                { range: 4100, elevation: 1012, timeOfFlight: 39.3, dElev100: "13m" },
                { range: 4200, elevation: 983, timeOfFlight: 38.6, dElev100: "15m" },
                { range: 4300, elevation: 953, timeOfFlight: 37.9, dElev100: "18m" },
                { range: 4400, elevation: 916, timeOfFlight: 36.9, dElev100: "20m" },
                { range: 4500, elevation: 870, timeOfFlight: 35.6, dElev100: "29m" },
                { range: 4600, elevation: 801, timeOfFlight: 33.6, dElev100: "0m" }
            ],
            // Charge 4 - MOD Adult Mortars (4 Rings) - อัพเดทตามข้อมูลล่าสุด
            4: [
                { range: 900, elevation: 1522, timeOfFlight: 53.7, dElev100: "5m" },
                { range: 1000, elevation: 1513, timeOfFlight: 53.7, dElev100: "5m" },
                { range: 1100, elevation: 1504, timeOfFlight: 53.6, dElev100: "5m" },
                { range: 1200, elevation: 1495, timeOfFlight: 53.5, dElev100: "5m" },
                { range: 1300, elevation: 1486, timeOfFlight: 53.5, dElev100: "5m" },
                { range: 1400, elevation: 1477, timeOfFlight: 53.5, dElev100: "5m" },
                { range: 1500, elevation: 1468, timeOfFlight: 53.4, dElev100: "5m" },
                { range: 1600, elevation: 1458, timeOfFlight: 53.4, dElev100: "4m" },
                { range: 1700, elevation: 1449, timeOfFlight: 53.3, dElev100: "4m" },
                { range: 1800, elevation: 1440, timeOfFlight: 53.2, dElev100: "5m" },
                { range: 1900, elevation: 1430, timeOfFlight: 53.1, dElev100: "4m" },
                { range: 2000, elevation: 1421, timeOfFlight: 53.1, dElev100: "5m" },
                { range: 2100, elevation: 1411, timeOfFlight: 53.0, dElev100: "4m" },
                { range: 2200, elevation: 1402, timeOfFlight: 52.9, dElev100: "5m" },
                { range: 2300, elevation: 1392, timeOfFlight: 52.8, dElev100: "5m" },
                { range: 2400, elevation: 1382, timeOfFlight: 52.7, dElev100: "5m" },
                { range: 2500, elevation: 1372, timeOfFlight: 52.6, dElev100: "5m" },
                { range: 2600, elevation: 1362, timeOfFlight: 52.4, dElev100: "5m" },
                { range: 2700, elevation: 1352, timeOfFlight: 52.3, dElev100: "5m" },
                { range: 2800, elevation: 1341, timeOfFlight: 52.2, dElev100: "5m" },
                { range: 2900, elevation: 1331, timeOfFlight: 52.0, dElev100: "5m" },
                { range: 3000, elevation: 1320, timeOfFlight: 51.9, dElev100: "5m" },
                { range: 3100, elevation: 1310, timeOfFlight: 51.7, dElev100: "5m" },
                { range: 3200, elevation: 1299, timeOfFlight: 51.6, dElev100: "6m" },
                { range: 3300, elevation: 1288, timeOfFlight: 51.4, dElev100: "6m" },
                { range: 3400, elevation: 1276, timeOfFlight: 51.2, dElev100: "6m" },
                { range: 3500, elevation: 1265, timeOfFlight: 51.1, dElev100: "5m" },
                { range: 3600, elevation: 1253, timeOfFlight: 50.9, dElev100: "6m" },
                { range: 3700, elevation: 1241, timeOfFlight: 50.7, dElev100: "6m" },
                { range: 3800, elevation: 1229, timeOfFlight: 50.4, dElev100: "6m" },
                { range: 3900, elevation: 1217, timeOfFlight: 50.2, dElev100: "7m" },
                { range: 4000, elevation: 1204, timeOfFlight: 50.0, dElev100: "6m" },
                { range: 4100, elevation: 1191, timeOfFlight: 49.7, dElev100: "7m" },
                { range: 4200, elevation: 1178, timeOfFlight: 49.5, dElev100: "7m" },
                { range: 4300, elevation: 1164, timeOfFlight: 49.2, dElev100: "7m" },
                { range: 4400, elevation: 1150, timeOfFlight: 48.9, dElev100: "7m" },
                { range: 4500, elevation: 1136, timeOfFlight: 48.6, dElev100: "7m" },
                { range: 4600, elevation: 1121, timeOfFlight: 48.2, dElev100: "8m" },
                { range: 4700, elevation: 1105, timeOfFlight: 47.9, dElev100: "8m" },
                { range: 4800, elevation: 1089, timeOfFlight: 47.5, dElev100: "8m" },
                { range: 4900, elevation: 1072, timeOfFlight: 47.1, dElev100: "8m" },
                { range: 5000, elevation: 1055, timeOfFlight: 46.7, dElev100: "9m" },
                { range: 5100, elevation: 1036, timeOfFlight: 46.2, dElev100: "9m" },
                { range: 5200, elevation: 1017, timeOfFlight: 45.7, dElev100: "10m" },
                { range: 5300, elevation: 996, timeOfFlight: 45.1, dElev100: "11m" },
                { range: 5400, elevation: 974, timeOfFlight: 44.5, dElev100: "12m" },
                { range: 5500, elevation: 950, timeOfFlight: 43.8, dElev100: "13m" },
                { range: 5600, elevation: 923, timeOfFlight: 43.0, dElev100: "15m" },
                { range: 5700, elevation: 892, timeOfFlight: 42.1, dElev100: "17m" },
                { range: 5800, elevation: 854, timeOfFlight: 40.9, dElev100: "25m" },
                { range: 5900, elevation: 802, timeOfFlight: 39.1, dElev100: "0m" }
            ]
        },
        // M819 smoke shell data
        M819: {
            1: [
                { range: 300, elevation: 1498, timeOfFlight: 25.6, dElev100: "18m" },
                { range: 350, elevation: 1480, timeOfFlight: 25.6, dElev100: "17m" },
                { range: 400, elevation: 1463, timeOfFlight: 25.5, dElev100: "18m" },
                { range: 450, elevation: 1445, timeOfFlight: 25.5, dElev100: "18m" },
                { range: 500, elevation: 1427, timeOfFlight: 25.4, dElev100: "18m" },
                { range: 550, elevation: 1409, timeOfFlight: 25.3, dElev100: "19m" },
                { range: 600, elevation: 1390, timeOfFlight: 25.2, dElev100: "19m" },
                { range: 650, elevation: 1371, timeOfFlight: 25.1, dElev100: "19m" },
                { range: 700, elevation: 1352, timeOfFlight: 25.0, dElev100: "20m" },
                { range: 750, elevation: 1332, timeOfFlight: 24.9, dElev100: "20m" },
                { range: 800, elevation: 1312, timeOfFlight: 24.7, dElev100: "21m" },
                { range: 850, elevation: 1291, timeOfFlight: 24.6, dElev100: "22m" },
                { range: 900, elevation: 1269, timeOfFlight: 24.4, dElev100: "22m" },
                { range: 950, elevation: 1247, timeOfFlight: 24.3, dElev100: "23m" },
                { range: 1000, elevation: 1224, timeOfFlight: 24.0, dElev100: "24m" },
                { range: 1050, elevation: 1200, timeOfFlight: 23.8, dElev100: "26m" },
                { range: 1100, elevation: 1174, timeOfFlight: 23.6, dElev100: "36m" },
                { range: 1150, elevation: 1148, timeOfFlight: 23.3, dElev100: "29m" },
                { range: 1200, elevation: 1119, timeOfFlight: 23.0, dElev100: "30m" },
                { range: 1250, elevation: 1089, timeOfFlight: 22.6, dElev100: "34m" },
                { range: 1300, elevation: 1055, timeOfFlight: 22.2, dElev100: "38m" },
                { range: 1350, elevation: 1017, timeOfFlight: 21.7, dElev100: "45m" },
                { range: 1400, elevation: 972, timeOfFlight: 21.1, dElev100: "56m" },
                { range: 1450, elevation: 916, timeOfFlight: 20.3, dElev100: "105m" },
                { range: 1500, elevation: 811, timeOfFlight: 18.6, dElev100: "0m" }
            ],
            // Charge 2 - MOD Adult Mortars (2 Rings) - Average Dispersion: 33m
            2: [
                { range: 400, elevation: 1527, timeOfFlight: 36.3, dElev100: "9m" },
                { range: 500, elevation: 1509, timeOfFlight: 36.3, dElev100: "10m" },
                { range: 600, elevation: 1490, timeOfFlight: 36.2, dElev100: "10m" },
                { range: 700, elevation: 1471, timeOfFlight: 36.1, dElev100: "10m" },
                { range: 800, elevation: 1452, timeOfFlight: 36.0, dElev100: "10m" },
                { range: 900, elevation: 1432, timeOfFlight: 35.9, dElev100: "10m" },
                { range: 1000, elevation: 1412, timeOfFlight: 35.8, dElev100: "10m" },
                { range: 1100, elevation: 1392, timeOfFlight: 35.7, dElev100: "10m" },
                { range: 1200, elevation: 1372, timeOfFlight: 35.5, dElev100: "11m" },
                { range: 1300, elevation: 1351, timeOfFlight: 35.4, dElev100: "11m" },
                { range: 1400, elevation: 1328, timeOfFlight: 35.2, dElev100: "11m" },
                { range: 1500, elevation: 1306, timeOfFlight: 34.9, dElev100: "11m" },
                { range: 1600, elevation: 1283, timeOfFlight: 34.7, dElev100: "12m" },
                { range: 1700, elevation: 1259, timeOfFlight: 34.4, dElev100: "12m" },
                { range: 1800, elevation: 1235, timeOfFlight: 34.2, dElev100: "13m" },
                { range: 1900, elevation: 1209, timeOfFlight: 33.8, dElev100: "13m" },
                { range: 2000, elevation: 1181, timeOfFlight: 33.5, dElev100: "14m" },
                { range: 2100, elevation: 1153, timeOfFlight: 33.1, dElev100: "15m" },
                { range: 2200, elevation: 1122, timeOfFlight: 32.6, dElev100: "16m" },
                { range: 2300, elevation: 1089, timeOfFlight: 32.1, dElev100: "18m" },
                { range: 2400, elevation: 1053, timeOfFlight: 31.5, dElev100: "20m" },
                { range: 2500, elevation: 1012, timeOfFlight: 30.7, dElev100: "22m" },
                { range: 2600, elevation: 965, timeOfFlight: 29.8, dElev100: "27m" },
                { range: 2700, elevation: 906, timeOfFlight: 28.7, dElev100: "39m" },
                { range: 2800, elevation: 810, timeOfFlight: 26.5, dElev100: "0m" }
            ],
            ...
            3: [
                { range: 500, elevation: 1534, timeOfFlight: 44.1, dElev100: "7m" },
                { range: 600, elevation: 1521, timeOfFlight: 44.1, dElev100: "7m" },
                { range: 700, elevation: 1507, timeOfFlight: 44.0, dElev100: "6m" },
                { range: 800, elevation: 1494, timeOfFlight: 44.0, dElev100: "7m" },
                { range: 900, elevation: 1480, timeOfFlight: 43.9, dElev100: "7m" },
                { range: 1000, elevation: 1466, timeOfFlight: 43.9, dElev100: "7m" },
                { range: 1100, elevation: 1452, timeOfFlight: 43.8, dElev100: "7m" },
                { range: 1200, elevation: 1438, timeOfFlight: 43.7, dElev100: "7m" },
                { range: 1300, elevation: 1424, timeOfFlight: 43.6, dElev100: "7m" },
                { range: 1400, elevation: 1409, timeOfFlight: 43.5, dElev100: "7m" },
                { range: 1500, elevation: 1395, timeOfFlight: 43.4, dElev100: "8m" },
                { range: 1600, elevation: 1380, timeOfFlight: 43.2, dElev100: "8m" },
                { range: 1700, elevation: 1364, timeOfFlight: 43.1, dElev100: "7m" },
                { range: 1800, elevation: 1349, timeOfFlight: 42.9, dElev100: "8m" },
                { range: 1900, elevation: 1333, timeOfFlight: 42.8, dElev100: "8m" },
                { range: 2000, elevation: 1317, timeOfFlight: 42.6, dElev100: "8m" },
                { range: 2100, elevation: 1300, timeOfFlight: 42.4, dElev100: "8m" },
                { range: 2200, elevation: 1284, timeOfFlight: 42.2, dElev100: "9m" },
                { range: 2300, elevation: 1266, timeOfFlight: 42, dElev100: "9m" },
                { range: 2400, elevation: 1249, timeOfFlight: 41.7, dElev100: "10m" },
                { range: 2500, elevation: 1230, timeOfFlight: 41.4, dElev100: "9m" },
                { range: 2600, elevation: 1211, timeOfFlight: 41.2, dElev100: "9m" },
                { range: 2700, elevation: 1192, timeOfFlight: 40.9, dElev100: "10m" },
                { range: 2800, elevation: 1172, timeOfFlight: 40.5, dElev100: "11m" },
                { range: 2900, elevation: 1150, timeOfFlight: 40.2, dElev100: "11m" },
                { range: 3000, elevation: 1128, timeOfFlight: 39.8, dElev100: "11m" },
                { range: 3100, elevation: 1105, timeOfFlight: 39.3, dElev100: "12m" },
                { range: 3200, elevation: 1080, timeOfFlight: 38.9, dElev100: "13m" },
                { range: 3300, elevation: 1054, timeOfFlight: 38.3, dElev100: "14m" },
                { range: 3400, elevation: 1026, timeOfFlight: 37.7, dElev100: "15m" },
                { range: 3500, elevation: 996, timeOfFlight: 37.1, dElev100: "18m" },
                { range: 3600, elevation: 961, timeOfFlight: 36.3, dElev100: "19m" },
                { range: 3700, elevation: 921, timeOfFlight: 35.3, dElev100: "21m" },
                { range: 3800, elevation: 873, timeOfFlight: 34.1, dElev100: "28m" },
                { range: 3900, elevation: 806, timeOfFlight: 32.3, dElev100: "0m" }
            ],
            // M819 Charge 4 - MOD Adult Mortars (4 Rings) - Average Dispersion: 57m
            4: [
                { range: 800, elevation: 1515, timeOfFlight: 50.7, dElev100: "6m" },
                { range: 900, elevation: 1504, timeOfFlight: 50.6, dElev100: "5m" },
                { range: 1000, elevation: 1493, timeOfFlight: 50.6, dElev100: "5m" },
                { range: 1100, elevation: 1482, timeOfFlight: 50.5, dElev100: "5m" },
                { range: 1200, elevation: 1471, timeOfFlight: 50.4, dElev100: "6m" },
                { range: 1300, elevation: 1460, timeOfFlight: 50.4, dElev100: "6m" },
                { range: 1400, elevation: 1448, timeOfFlight: 50.3, dElev100: "5m" },
                { range: 1500, elevation: 1437, timeOfFlight: 50.2, dElev100: "6m" },
                { range: 1600, elevation: 1425, timeOfFlight: 50.1, dElev100: "5m" },
                { range: 1700, elevation: 1414, timeOfFlight: 50.0, dElev100: "6m" },
                { range: 1800, elevation: 1402, timeOfFlight: 49.9, dElev100: "6m" },
                { range: 1900, elevation: 1390, timeOfFlight: 49.8, dElev100: "6m" },
                { range: 2000, elevation: 1378, timeOfFlight: 49.7, dElev100: "6m" },
                { range: 2100, elevation: 1366, timeOfFlight: 49.5, dElev100: "7m" },
                { range: 2200, elevation: 1353, timeOfFlight: 49.4, dElev100: "6m" },
                { range: 2300, elevation: 1340, timeOfFlight: 49.2, dElev100: "6m" },
                { range: 2400, elevation: 1328, timeOfFlight: 49.1, dElev100: "7m" },
                { range: 2500, elevation: 1314, timeOfFlight: 48.9, dElev100: "6m" },
                { range: 2600, elevation: 1301, timeOfFlight: 48.7, dElev100: "7m" },
                { range: 2700, elevation: 1288, timeOfFlight: 48.5, dElev100: "7m" },
                { range: 2800, elevation: 1274, timeOfFlight: 48.3, dElev100: "7m" },
                { range: 2900, elevation: 1260, timeOfFlight: 48.1, dElev100: "7m" },
                { range: 3000, elevation: 1245, timeOfFlight: 47.9, dElev100: "7m" },
                { range: 3100, elevation: 1230, timeOfFlight: 47.7, dElev100: "7m" },
                { range: 3200, elevation: 1215, timeOfFlight: 47.4, dElev100: "7m" },
                { range: 3300, elevation: 1200, timeOfFlight: 47.1, dElev100: "8m" },
                { range: 3400, elevation: 1184, timeOfFlight: 46.8, dElev100: "8m" },
                { range: 3500, elevation: 1167, timeOfFlight: 46.5, dElev100: "8m" },
                { range: 3600, elevation: 1151, timeOfFlight: 46.2, dElev100: "9m" },
                { range: 3700, elevation: 1133, timeOfFlight: 45.8, dElev100: "9m" },
                { range: 3800, elevation: 1115, timeOfFlight: 45.5, dElev100: "10m" },
                { range: 3900, elevation: 1096, timeOfFlight: 45.1, dElev100: "10m" },
                { range: 4000, elevation: 1076, timeOfFlight: 44.6, dElev100: "10m" },
                { range: 4100, elevation: 1055, timeOfFlight: 44.1, dElev100: "10m" },
                { range: 4200, elevation: 1033, timeOfFlight: 43.6, dElev100: "11m" },
                { range: 4300, elevation: 1010, timeOfFlight: 43.1, dElev100: "12m" },
                { range: 4400, elevation: 985, timeOfFlight: 42.4, dElev100: "13m" },
                { range: 4500, elevation: 958, timeOfFlight: 41.7, dElev100: "14m" },
                { range: 4600, elevation: 929, timeOfFlight: 40.9, dElev100: "16m" },
                { range: 4700, elevation: 895, timeOfFlight: 40.0, dElev100: "20m" },
                { range: 4800, elevation: 856, timeOfFlight: 38.8, dElev100: "25m" },
                { range: 4900, elevation: 805, timeOfFlight: 37.3, dElev100: "0m" }
            ]
        },
        M853A1: {
            // 1 Ring - M853A1 illumination round (1 Ring)
            1: [
                { range: 300, elevation: 1493, timeOfFlight: 26.1, dElev100: "18m" },
                { range: 350, elevation: 1475, timeOfFlight: 26.0, dElev100: "18m" },
                { range: 400, elevation: 1457, timeOfFlight: 26.0, dElev100: "19m" },
                { range: 450, elevation: 1438, timeOfFlight: 25.9, dElev100: "19m" },
                { range: 500, elevation: 1419, timeOfFlight: 25.8, dElev100: "19m" },
                { range: 550, elevation: 1400, timeOfFlight: 25.7, dElev100: "21m" },
                { range: 600, elevation: 1379, timeOfFlight: 25.6, dElev100: "20m" },
                { range: 650, elevation: 1359, timeOfFlight: 25.5, dElev100: "21m" },
                { range: 700, elevation: 1338, timeOfFlight: 25.4, dElev100: "21m" },
                { range: 750, elevation: 1317, timeOfFlight: 25.2, dElev100: "22m" },
                { range: 800, elevation: 1295, timeOfFlight: 25.1, dElev100: "23m" },
                { range: 850, elevation: 1272, timeOfFlight: 24.9, dElev100: "23m" },
                { range: 900, elevation: 1249, timeOfFlight: 24.7, dElev100: "25m" },
                { range: 950, elevation: 1224, timeOfFlight: 24.5, dElev100: "25m" },
                { range: 1000, elevation: 1199, timeOfFlight: 24.3, dElev100: "27m" },
                { range: 1050, elevation: 1172, timeOfFlight: 24.0, dElev100: "28m" },
                { range: 1100, elevation: 1144, timeOfFlight: 23.7, dElev100: "33m" },
                { range: 1150, elevation: 1114, timeOfFlight: 23.4, dElev100: "35m" },
                { range: 1200, elevation: 1081, timeOfFlight: 23, dElev100: "41m" },
                { range: 1250, elevation: 1046, timeOfFlight: 22.6, dElev100: "47m" },
                { range: 1300, elevation: 1005, timeOfFlight: 22.1, dElev100: "58m" },
                { range: 1350, elevation: 958, timeOfFlight: 21.4, dElev100: "98m" },
                { range: 1400, elevation: 900, timeOfFlight: 20.6, dElev100: "0m" },
                { range: 1450, elevation: 802, timeOfFlight: 19, dElev100: "0m" }
            ],
            // 2 Ring - M853A1 illumination round (2 Rings)
            2: [
                { range: 500, elevation: 1509, timeOfFlight: 39.8, dElev100: "9m" },
                { range: 600, elevation: 1490, timeOfFlight: 39.7, dElev100: "9m" },
                { range: 700, elevation: 1470, timeOfFlight: 39.6, dElev100: "10m" },
                { range: 800, elevation: 1452, timeOfFlight: 39.5, dElev100: "10m" },
                { range: 900, elevation: 1433, timeOfFlight: 39.3, dElev100: "10m" },
                { range: 1000, elevation: 1413, timeOfFlight: 39.3, dElev100: "10m" },
                { range: 1100, elevation: 1393, timeOfFlight: 39.1, dElev100: "11m" },
                { range: 1200, elevation: 1372, timeOfFlight: 39.0, dElev100: "11m" },
                { range: 1300, elevation: 1351, timeOfFlight: 38.8, dElev100: "11m" },
                { range: 1400, elevation: 1329, timeOfFlight: 38.6, dElev100: "11m" },
                { range: 1500, elevation: 1306, timeOfFlight: 38.3, dElev100: "11m" },
                { range: 1600, elevation: 1283, timeOfFlight: 38.1, dElev100: "12m" },
                { range: 1700, elevation: 1259, timeOfFlight: 37.8, dElev100: "12m" },
                { range: 1800, elevation: 1235, timeOfFlight: 37.5, dElev100: "13m" },
                { range: 1900, elevation: 1209, timeOfFlight: 37.2, dElev100: "13m" },
                { range: 2000, elevation: 1182, timeOfFlight: 36.8, dElev100: "14m" },
                { range: 2100, elevation: 1154, timeOfFlight: 36.5, dElev100: "15m" },
                { range: 2200, elevation: 1125, timeOfFlight: 35.9, dElev100: "17m" },
                { range: 2300, elevation: 1093, timeOfFlight: 35.4, dElev100: "18m" },
                { range: 2400, elevation: 1058, timeOfFlight: 34.8, dElev100: "19m" },
                { range: 2500, elevation: 1021, timeOfFlight: 34.1, dElev100: "22m" },
                { range: 2600, elevation: 980, timeOfFlight: 33.3, dElev100: "26m" },
                { range: 2700, elevation: 934, timeOfFlight: 32.3, dElev100: "32m" },
                { range: 2800, elevation: 878, timeOfFlight: 31.1, dElev100: "32m" },
                { range: 2900, elevation: 806, timeOfFlight: 29.4, dElev100: "0m" }
            ],
            // 3 Ring - M853A1 illumination round (3 Rings)
            3: [
                { range: 700, elevation: 1507, timeOfFlight: 50.1, dElev100: "7m" },
                { range: 800, elevation: 1493, timeOfFlight: 50.0, dElev100: "7m" },
                { range: 900, elevation: 1479, timeOfFlight: 50.0, dElev100: "7m" },
                { range: 1000, elevation: 1465, timeOfFlight: 49.9, dElev100: "7m" },
                { range: 1100, elevation: 1451, timeOfFlight: 49.8, dElev100: "7m" },
                { range: 1200, elevation: 1437, timeOfFlight: 49.7, dElev100: "7m" },
                { range: 1300, elevation: 1422, timeOfFlight: 49.6, dElev100: "7m" },
                { range: 1400, elevation: 1408, timeOfFlight: 49.5, dElev100: "8m" },
                { range: 1500, elevation: 1393, timeOfFlight: 49.3, dElev100: "8m" },
                { range: 1600, elevation: 1378, timeOfFlight: 49.2, dElev100: "8m" },
                { range: 1700, elevation: 1362, timeOfFlight: 49.0, dElev100: "7m" },
                { range: 1800, elevation: 1347, timeOfFlight: 48.8, dElev100: "8m" },
                { range: 1900, elevation: 1331, timeOfFlight: 48.7, dElev100: "8m" },
                { range: 2000, elevation: 1315, timeOfFlight: 48.5, dElev100: "9m" },
                { range: 2100, elevation: 1298, timeOfFlight: 48.2, dElev100: "8m" },
                { range: 2200, elevation: 1281, timeOfFlight: 48.0, dElev100: "8m" },
                { range: 2300, elevation: 1264, timeOfFlight: 47.8, dElev100: "9m" },
                { range: 2400, elevation: 1246, timeOfFlight: 47.5, dElev100: "9m" },
                { range: 2500, elevation: 1227, timeOfFlight: 47.2, dElev100: "9m" },
                { range: 2600, elevation: 1210, timeOfFlight: 46.9, dElev100: "10m" },
                { range: 2700, elevation: 1191, timeOfFlight: 46.6, dElev100: "10m" },
                { range: 2800, elevation: 1171, timeOfFlight: 46.2, dElev100: "10m" },
                { range: 2900, elevation: 1153, timeOfFlight: 45.9, dElev100: "11m" },
                { range: 3000, elevation: 1133, timeOfFlight: 45.5, dElev100: "11m" },
                { range: 3100, elevation: 1108, timeOfFlight: 45.0, dElev100: "11m" },
                { range: 3200, elevation: 1086, timeOfFlight: 44.5, dElev100: "12m" },
                { range: 3300, elevation: 1062, timeOfFlight: 44.0, dElev100: "13m" },
                { range: 3400, elevation: 1038, timeOfFlight: 43.5, dElev100: "13m" },
                { range: 3500, elevation: 1012, timeOfFlight: 42.9, dElev100: "14m" },
                { range: 3600, elevation: 984, timeOfFlight: 42.2, dElev100: "14m" },
                { range: 3700, elevation: 955, timeOfFlight: 41.5, dElev100: "16m" },
                { range: 3800, elevation: 923, timeOfFlight: 40.7, dElev100: "17m" },
                { range: 3900, elevation: 888, timeOfFlight: 39.8, dElev100: "18m" },
                { range: 4000, elevation: 850, timeOfFlight: 38.7, dElev100: "23m" },
                { range: 4100, elevation: 804, timeOfFlight: 37.4, dElev100: "0m" }
            ],
            // 4 Ring - M853A1 illumination round (4 Rings)
            4: [
                { range: 3400, elevation: 1162, timeOfFlight: 53.0, dElev100: "9m" },
                { range: 3500, elevation: 1145, timeOfFlight: 52.7, dElev100: "9m" },
                { range: 3600, elevation: 1128, timeOfFlight: 52.3, dElev100: "9m" },
                { range: 3700, elevation: 1110, timeOfFlight: 51.9, dElev100: "9m" },
                { range: 3800, elevation: 1092, timeOfFlight: 51.5, dElev100: "9m" },
                { range: 3900, elevation: 1073, timeOfFlight: 51.0, dElev100: "9m" },
                { range: 4000, elevation: 1054, timeOfFlight: 50.5, dElev100: "10m" },
                { range: 4100, elevation: 1034, timeOfFlight: 50.0, dElev100: "10m" },
                { range: 4200, elevation: 1013, timeOfFlight: 49.5, dElev100: "10m" },
                { range: 4300, elevation: 992, timeOfFlight: 48.9, dElev100: "12m" },
                { range: 4400, elevation: 969, timeOfFlight: 48.3, dElev100: "12m" },
                { range: 4500, elevation: 946, timeOfFlight: 47.6, dElev100: "13m" },
                { range: 4600, elevation: 921, timeOfFlight: 46.9, dElev100: "13m" },
                { range: 4700, elevation: 894, timeOfFlight: 46.1, dElev100: "14m" },
                { range: 4800, elevation: 866, timeOfFlight: 45.2, dElev100: "15m" },
                { range: 4900, elevation: 835, timeOfFlight: 44.2, dElev100: "16m" },
                { range: 5000, elevation: 801, timeOfFlight: 43.1, dElev100: "0m" }
            ]
        },
        "0-832Ay": {
            // 0 Ring - MOD (Dispersion: 13m ตามภาพใหม่)
            0: [
                { range: 50, elevation: 1455, timeOfFlight: 15.0, dElev100: "44m" },
                { range: 100, elevation: 1411, timeOfFlight: 15.0, dElev100: "46m" },
                { range: 150, elevation: 1365, timeOfFlight: 14.9, dElev100: "47m" },
                { range: 200, elevation: 1318, timeOfFlight: 14.8, dElev100: "50m" },
                { range: 250, elevation: 1268, timeOfFlight: 14.6, dElev100: "51m" },
                { range: 300, elevation: 1217, timeOfFlight: 14.4, dElev100: "58m" },
            ...
            1: [
                { range: 200, elevation: 1435, timeOfFlight: 25.5, dElev100: "16m" },
                { range: 300, elevation: 1403, timeOfFlight: 25.4, dElev100: "17m" },
                { range: 400, elevation: 1369, timeOfFlight: 25.3, dElev100: "17m" },
                { range: 500, elevation: 1335, timeOfFlight: 25.2, dElev100: "18m" },
                { range: 600, elevation: 1299, timeOfFlight: 25, dElev100: "18m" },
                { range: 700, elevation: 1263, timeOfFlight: 24.8, dElev100: "19m" },
                { range: 800, elevation: 1224, timeOfFlight: 24.5, dElev100: "20m" },
                { range: 900, elevation: 1183, timeOfFlight: 24.2, dElev100: "21m" },
                { range: 1000, elevation: 1139, timeOfFlight: 23.8, dElev100: "23m" },
                { range: 1100, elevation: 1091, timeOfFlight: 23.3, dElev100: "26m" },
                { range: 1200, elevation: 1037, timeOfFlight: 22.7, dElev100: "31m" },
                { range: 1300, elevation: 973, timeOfFlight: 21.9, dElev100: "38m" },
                { range: 1400, elevation: 883, timeOfFlight: 20.5, dElev100: "68m" }
            ],
            // 2 Ring - MOD (Dispersion: 33m ตามภาพที่ 2)
            2: [
                { range: 300, elevation: 1439, timeOfFlight: 32.9, dElev100: "11m" },
                { range: 400, elevation: 1418, timeOfFlight: 32.8, dElev100: "11m" },
                { range: 500, elevation: 1397, timeOfFlight: 32.7, dElev100: "11m" },
                { range: 600, elevation: 1375, timeOfFlight: 32.6, dElev100: "10m" },
                { range: 700, elevation: 1354, timeOfFlight: 32.5, dElev100: "11m" },
                { range: 800, elevation: 1332, timeOfFlight: 32.4, dElev100: "11m" },
                { range: 900, elevation: 1309, timeOfFlight: 32.3, dElev100: "11m" },
                { range: 1000, elevation: 1286, timeOfFlight: 32.1, dElev100: "12m" },
                { range: 1100, elevation: 1263, timeOfFlight: 31.9, dElev100: "13m" },
                { range: 1200, elevation: 1238, timeOfFlight: 31.7, dElev100: "13m" },
                { range: 1300, elevation: 1213, timeOfFlight: 31.5, dElev100: "14m" },
                { range: 1400, elevation: 1186, timeOfFlight: 31.2, dElev100: "13m" },
                { range: 1500, elevation: 1158, timeOfFlight: 30.9, dElev100: "14m" },
                { range: 1600, elevation: 1130, timeOfFlight: 30.5, dElev100: "16m" },
                { range: 1700, elevation: 1098, timeOfFlight: 30.1, dElev100: "16m" },
                { range: 1800, elevation: 1065, timeOfFlight: 29.7, dElev100: "17m" },
                { range: 1900, elevation: 1030, timeOfFlight: 29.1, dElev100: "20m" },
                { range: 2000, elevation: 989, timeOfFlight: 28.5, dElev100: "22m" },
                { range: 2100, elevation: 943, timeOfFlight: 27.7, dElev100: "27m" },
                { range: 2200, elevation: 887, timeOfFlight: 26.6, dElev100: "36m" },
                { range: 2300, elevation: 805, timeOfFlight: 24.9, dElev100: "0m" }
            ],
            // 3 Ring - MOD (Dispersion: 44m ตามภาพที่ 2)
            3: [
                { range: 400, elevation: 1438, timeOfFlight: 38.7, dElev100: "8m" },
                { range: 500, elevation: 1422, timeOfFlight: 38.7, dElev100: "7m" },
                { range: 600, elevation: 1407, timeOfFlight: 38.6, dElev100: "8m" },
                { range: 700, elevation: 1391, timeOfFlight: 38.5, dElev100: "8m" },
                { range: 800, elevation: 1374, timeOfFlight: 38.5, dElev100: "8m" },
                { range: 900, elevation: 1358, timeOfFlight: 38.4, dElev100: "8m" },
                { range: 1000, elevation: 1342, timeOfFlight: 38.3, dElev100: "9m" },
                { range: 1100, elevation: 1325, timeOfFlight: 38.1, dElev100: "9m" },
                { range: 1200, elevation: 1307, timeOfFlight: 38, dElev100: "8m" },
                { range: 1300, elevation: 1290, timeOfFlight: 37.9, dElev100: "8m" },
                { range: 1400, elevation: 1272, timeOfFlight: 37.7, dElev100: "9m" },
                { range: 1500, elevation: 1254, timeOfFlight: 37.5, dElev100: "10m" },
                { range: 1600, elevation: 1235, timeOfFlight: 37.3, dElev100: "10m" },
                { range: 1700, elevation: 1216, timeOfFlight: 37.1, dElev100: "10m" },
                { range: 1800, elevation: 1196, timeOfFlight: 36.9, dElev100: "11m" },
                { range: 1900, elevation: 1175, timeOfFlight: 36.6, dElev100: "11m" },
                { range: 2000, elevation: 1153, timeOfFlight: 36.3, dElev100: "11m" },
                { range: 2100, elevation: 1131, timeOfFlight: 36, dElev100: "11m" },
                { range: 2200, elevation: 1108, timeOfFlight: 35.7, dElev100: "12m" },
                { range: 2300, elevation: 1084, timeOfFlight: 35.3, dElev100: "13m" },
                { range: 2400, elevation: 1058, timeOfFlight: 34.8, dElev100: "14m" },
                { range: 2500, elevation: 1031, timeOfFlight: 34.4, dElev100: "15m" },
                { range: 2600, elevation: 1001, timeOfFlight: 33.8, dElev100: "15m" },
                { range: 2700, elevation: 968, timeOfFlight: 33.2, dElev100: "17m" },
                { range: 2800, elevation: 933, timeOfFlight: 32.4, dElev100: "21m" },
                { range: 2900, elevation: 891, timeOfFlight: 31.5, dElev100: "26m" },
                { range: 3000, elevation: 839, timeOfFlight: 30.3, dElev100: "33m" },
                { range: 3100, elevation: 758, timeOfFlight: 28.3, dElev100: "0m" }
            ],
            // 4 Ring - MOD (Dispersion: 55m ตามภาพที่ 2)
            4: [
                { range: 600, elevation: 1426, timeOfFlight: 44.4, dElev100: "6m" },
                { range: 700, elevation: 1413, timeOfFlight: 44.4, dElev100: "6m" },
                { range: 800, elevation: 1401, timeOfFlight: 44.3, dElev100: "7m" },
                { range: 900, elevation: 1388, timeOfFlight: 44.2, dElev100: "7m" },
                { range: 1000, elevation: 1375, timeOfFlight: 44.2, dElev100: "7m" },
                { range: 1100, elevation: 1362, timeOfFlight: 44.1, dElev100: "7m" },
                { range: 1200, elevation: 1348, timeOfFlight: 44, dElev100: "6m" },
                { range: 1300, elevation: 1335, timeOfFlight: 43.9, dElev100: "7m" },
                { range: 1400, elevation: 1321, timeOfFlight: 43.8, dElev100: "6m" },
                { range: 1500, elevation: 1308, timeOfFlight: 43.7, dElev100: "7m" },
                { range: 1600, elevation: 1294, timeOfFlight: 43.5, dElev100: "7m" },
                { range: 1700, elevation: 1279, timeOfFlight: 43.4, dElev100: "7m" },
                { range: 1800, elevation: 1265, timeOfFlight: 43.2, dElev100: "8m" },
                { range: 1900, elevation: 1250, timeOfFlight: 43.1, dElev100: "7m" },
                { range: 2000, elevation: 1235, timeOfFlight: 42.9, dElev100: "8m" },
                { range: 2100, elevation: 1220, timeOfFlight: 42.7, dElev100: "8m" },
                { range: 2200, elevation: 1204, timeOfFlight: 42.5, dElev100: "8m" },
                { range: 2300, elevation: 1188, timeOfFlight: 42.3, dElev100: "9m" },
                { range: 2400, elevation: 1171, timeOfFlight: 42, dElev100: "9m" },
                { range: 2500, elevation: 1154, timeOfFlight: 41.8, dElev100: "9m" },
                { range: 2600, elevation: 1136, timeOfFlight: 41.5, dElev100: "9m" },
                { range: 2700, elevation: 1118, timeOfFlight: 41.2, dElev100: "9m" },
                { range: 2800, elevation: 1099, timeOfFlight: 40.8, dElev100: "10m" },
                { range: 2900, elevation: 1077, timeOfFlight: 40.5, dElev100: "10m" },
                // แก้ค่าพิมพ์ผิด: เดิมเป็น 50.1 ซึ่งขัดกับแถวข้างเคียง (40.5 / 39.7) และการจำลองวิถี (40.1)
                { range: 3000, elevation: 1059, timeOfFlight: 40.1, dElev100: "11m" },
                { range: 3100, elevation: 1037, timeOfFlight: 39.7, dElev100: "11m" },
                { range: 3200, elevation: 1014, timeOfFlight: 39.2, dElev100: "12m" },
                { range: 3300, elevation: 990, timeOfFlight: 38.7, dElev100: "13m" },
                { range: 3400, elevation: 963, timeOfFlight: 38, dElev100: "13m" },
                { range: 3500, elevation: 936, timeOfFlight: 37.4, dElev100: "16m" },
                { range: 3600, elevation: 904, timeOfFlight: 36.6, dElev100: "17m" },
                { range: 3700, elevation: 868, timeOfFlight: 35.7, dElev100: "20m" },
                { range: 3800, elevation: 824, timeOfFlight: 34.5, dElev100: "24m" },
                { range: 3900, elevation: 766, timeOfFlight: 32.9, dElev100: "0m" }
            ]
        },
        "A-832AY": {
            ...
            0: [
                { range: 50, elevation: 1450, timeOfFlight: 14.1, dElev100: "51m" },
                { range: 100, elevation: 1399, timeOfFlight: 14.0, dElev100: "52m" },
                { range: 150, elevation: 1347, timeOfFlight: 13.9, dElev100: "55m" },
                { range: 200, elevation: 1292, timeOfFlight: 13.8, dElev100: "57m" },
                { range: 250, elevation: 1235, timeOfFlight: 13.6, dElev100: "63m" },
                { range: 300, elevation: 1172, timeOfFlight: 13.3, dElev100: "70m" },
            ...
            1: [
                { range: 200, elevation: 1429, timeOfFlight: 24.3, dElev100: "18m" },
                { range: 300, elevation: 1393, timeOfFlight: 24.2, dElev100: "18m" },
                { range: 400, elevation: 1357, timeOfFlight: 24.1, dElev100: "19m" },
                { range: 500, elevation: 1319, timeOfFlight: 23.9, dElev100: "20m" },
                { range: 600, elevation: 1279, timeOfFlight: 23.7, dElev100: "20m" },
                { range: 700, elevation: 1238, timeOfFlight: 23.4, dElev100: "21m" },
                { range: 800, elevation: 1194, timeOfFlight: 23.1, dElev100: "22m" },
                { range: 900, elevation: 1147, timeOfFlight: 22.7, dElev100: "24m" },
                { range: 1000, elevation: 1096, timeOfFlight: 22.2, dElev100: "28m" },
                { range: 1100, elevation: 1037, timeOfFlight: 21.6, dElev100: "34m" },
                { range: 1200, elevation: 966, timeOfFlight: 20.7, dElev100: "46m" },
                { range: 1300, elevation: 862, timeOfFlight: 19.2, dElev100: "106m" },
                { range: 1300, elevation: 862, timeOfFlight: 19.2, dElev100: "0m" }
            ],
            // 2 Ring - MOD A-832AY (ตามภาพใหม่ - Ring 3)
            2: [
                { range: 300, elevation: 1433, timeOfFlight: 31.1, dElev100: "12m" },
                { range: 400, elevation: 1410, timeOfFlight: 31.1, dElev100: "11m" },
                { range: 500, elevation: 1387, timeOfFlight: 31.0, dElev100: "12m" },
                { range: 600, elevation: 1364, timeOfFlight: 30.9, dElev100: "12m" },
                { range: 700, elevation: 1340, timeOfFlight: 30.8, dElev100: "12m" },
                { range: 800, elevation: 1316, timeOfFlight: 30.6, dElev100: "13m" },
                { range: 900, elevation: 1290, timeOfFlight: 30.5, dElev100: "12m" },
                { range: 1000, elevation: 1265, timeOfFlight: 30.3, dElev100: "14m" },
                { range: 1100, elevation: 1238, timeOfFlight: 30.1, dElev100: "14m" },
                { range: 1200, elevation: 1210, timeOfFlight: 29.8, dElev100: "14m" },
                { range: 1300, elevation: 1181, timeOfFlight: 29.5, dElev100: "14m" },
                { range: 1400, elevation: 1151, timeOfFlight: 29.2, dElev100: "16m" },
                { range: 1500, elevation: 1119, timeOfFlight: 28.8, dElev100: "17m" },
                { range: 1600, elevation: 1084, timeOfFlight: 28.4, dElev100: "18m" },
                { range: 1700, elevation: 1047, timeOfFlight: 27.8, dElev100: "20m" },
                { range: 1800, elevation: 1005, timeOfFlight: 27.2, dElev100: "23m" },
                { range: 1900, elevation: 956, timeOfFlight: 26.4, dElev100: "27m" },
                { range: 2000, elevation: 898, timeOfFlight: 25.4, dElev100: "38m" },
                { range: 2100, elevation: 810, timeOfFlight: 23.7, dElev100: "0m" }
            ],
            // 3 Ring - MOD A-832AY (ตามภาพใหม่ - Ring 4) 
            3: [
                { range: 400, elevation: 1432, timeOfFlight: 36.5, dElev100: "8m" },
                { range: 500, elevation: 1415, timeOfFlight: 36.5, dElev100: "8m" },
                { range: 600, elevation: 1398, timeOfFlight: 36.4, dElev100: "9m" },
                { range: 700, elevation: 1380, timeOfFlight: 36.3, dElev100: "9m" },
                { range: 800, elevation: 1362, timeOfFlight: 36.2, dElev100: "9m" },
                { range: 900, elevation: 1344, timeOfFlight: 36.1, dElev100: "9m" },
                { range: 1000, elevation: 1326, timeOfFlight: 36.0, dElev100: "9m" },
                { range: 1100, elevation: 1307, timeOfFlight: 35.9, dElev100: "9m" },
                { range: 1200, elevation: 1288, timeOfFlight: 35.7, dElev100: "10m" },
                { range: 1300, elevation: 1269, timeOfFlight: 35.6, dElev100: "11m" },
                { range: 1400, elevation: 1248, timeOfFlight: 35.4, dElev100: "10m" },
                { range: 1500, elevation: 1228, timeOfFlight: 35.2, dElev100: "11m" },
                { range: 1600, elevation: 1206, timeOfFlight: 34.9, dElev100: "11m" },
                { range: 1700, elevation: 1184, timeOfFlight: 34.7, dElev100: "11m" },
                { range: 1800, elevation: 1161, timeOfFlight: 34.4, dElev100: "12m" },
                { range: 1900, elevation: 1138, timeOfFlight: 34.1, dElev100: "12m" },
                { range: 2000, elevation: 1112, timeOfFlight: 33.7, dElev100: "13m" },
                { range: 2100, elevation: 1086, timeOfFlight: 33.3, dElev100: "13m" },
                { range: 2200, elevation: 1058, timeOfFlight: 32.9, dElev100: "15m" },
                { range: 2300, elevation: 1028, timeOfFlight: 32.4, dElev100: "16m" },
                { range: 2400, elevation: 995, timeOfFlight: 31.8, dElev100: "18m" },
                { range: 2500, elevation: 958, timeOfFlight: 31.1, dElev100: "19m" },
                { range: 2600, elevation: 916, timeOfFlight: 30.2, dElev100: "23m" },
                { range: 2700, elevation: 865, timeOfFlight: 29.1, dElev100: "32m" },
                { range: 2800, elevation: 792, timeOfFlight: 27.5, dElev100: "0m" }
            ]
        },
        "C-832C": {
            // Ring 1 - MOD C-832C Illumination (ตามภาพ Ring 1 - Dispersion: 15m)
            1: [
                { range: 150, elevation: 1429, timeOfFlight: 21.9, dElev100: "24m" },
                { range: 200, elevation: 1405, timeOfFlight: 21.8, dElev100: "24m" },
                { range: 250, elevation: 1381, timeOfFlight: 21.8, dElev100: "25m" },
                { range: 300, elevation: 1356, timeOfFlight: 21.7, dElev100: "25m" },
                { range: 350, elevation: 1331, timeOfFlight: 21.6, dElev100: "26m" },
                { range: 400, elevation: 1305, timeOfFlight: 21.5, dElev100: "27m" },
                { range: 450, elevation: 1278, timeOfFlight: 21.4, dElev100: "28m" },
                { range: 500, elevation: 1250, timeOfFlight: 21.2, dElev100: "28m" },
                { range: 550, elevation: 1222, timeOfFlight: 21.0, dElev100: "31m" },
                { range: 600, elevation: 1191, timeOfFlight: 20.8, dElev100: "32m" },
                { range: 650, elevation: 1159, timeOfFlight: 20.6, dElev100: "33m" },
                { range: 700, elevation: 1126, timeOfFlight: 20.3, dElev100: "36m" },
                { range: 750, elevation: 1090, timeOfFlight: 20.0, dElev100: "40m" },
                { range: 800, elevation: 1050, timeOfFlight: 19.6, dElev100: "45m" },
                { range: 850, elevation: 1005, timeOfFlight: 19.1, dElev100: "50m" },
                { range: 900, elevation: 955, timeOfFlight: 18.6, dElev100: "63m" },
                { range: 950, elevation: 892, timeOfFlight: 17.8, dElev100: "99m" },
                { range: 1000, elevation: 800, timeOfFlight: 16.6, dElev100: "0m" }
            ],
            // Ring 2 - MOD C-832C Illumination (ตามภาพ Ring 2 - Dispersion: 28m)
            2: [
                { range: 300, elevation: 1424, timeOfFlight: 32.2, dElev100: "13m" },
                { range: 400, elevation: 1398, timeOfFlight: 32.1, dElev100: "13m" },
                { range: 500, elevation: 1371, timeOfFlight: 32.0, dElev100: "13m" },
                { range: 600, elevation: 1344, timeOfFlight: 31.9, dElev100: "14m" },
                { range: 700, elevation: 1316, timeOfFlight: 31.7, dElev100: "14m" },
                { range: 800, elevation: 1287, timeOfFlight: 31.5, dElev100: "14m" },
                { range: 900, elevation: 1257, timeOfFlight: 31.3, dElev100: "15m" },
                { range: 1000, elevation: 1226, timeOfFlight: 31.0, dElev100: "16m" },
                { range: 1100, elevation: 1194, timeOfFlight: 30.7, dElev100: "17m" },
                { range: 1200, elevation: 1159, timeOfFlight: 30.4, dElev100: "18m" },
                { range: 1300, elevation: 1123, timeOfFlight: 30.0, dElev100: "19m" },
                { range: 1400, elevation: 1084, timeOfFlight: 29.5, dElev100: "21m" },
                { range: 1500, elevation: 1042, timeOfFlight: 28.9, dElev100: "23m" },
                { range: 1600, elevation: 995, timeOfFlight: 28.2, dElev100: "26m" },
                { range: 1700, elevation: 942, timeOfFlight: 27.3, dElev100: "30m" },
                { range: 1800, elevation: 879, timeOfFlight: 26.2, dElev100: "37m" },
                { range: 1900, elevation: 796, timeOfFlight: 24.6, dElev100: "0m" }
            ],
            // Ring 3 - MOD C-832C Illumination (ตามภาพ Ring 3 - Dispersion: 39m)  
            3: [
                { range: 400, elevation: 1426, timeOfFlight: 40.4, dElev100: "10m" },
                { range: 500, elevation: 1407, timeOfFlight: 40.3, dElev100: "10m" },
                { range: 600, elevation: 1387, timeOfFlight: 40.2, dElev100: "9m" },
                { range: 700, elevation: 1368, timeOfFlight: 40.1, dElev100: "10m" },
                { range: 800, elevation: 1348, timeOfFlight: 40.0, dElev100: "10m" },
                { range: 900, elevation: 1328, timeOfFlight: 39.9, dElev100: "11m" },
                { range: 1000, elevation: 1307, timeOfFlight: 39.7, dElev100: "11m" },
                { range: 1100, elevation: 1286, timeOfFlight: 39.5, dElev100: "11m" },
                { range: 1200, elevation: 1264, timeOfFlight: 39.3, dElev100: "11m" },
                { range: 1300, elevation: 1242, timeOfFlight: 39.1, dElev100: "12m" },
                { range: 1400, elevation: 1219, timeOfFlight: 38.9, dElev100: "12m" },
                { range: 1500, elevation: 1195, timeOfFlight: 38.6, dElev100: "12m" },
                { range: 1600, elevation: 1170, timeOfFlight: 38.3, dElev100: "12m" },
                { range: 1700, elevation: 1145, timeOfFlight: 37.9, dElev100: "13m" },
                { range: 1800, elevation: 1118, timeOfFlight: 37.5, dElev100: "13m" },
                { range: 1900, elevation: 1090, timeOfFlight: 37.1, dElev100: "14m" },
                { range: 2000, elevation: 1061, timeOfFlight: 36.6, dElev100: "15m" },
                { range: 2100, elevation: 1031, timeOfFlight: 36.1, dElev100: "17m" },
                { range: 2200, elevation: 997, timeOfFlight: 35.5, dElev100: "17m" },
                { range: 2300, elevation: 962, timeOfFlight: 34.8, dElev100: "19m" },
                { range: 2400, elevation: 923, timeOfFlight: 34.0, dElev100: "21m" },
                { range: 2500, elevation: 880, timeOfFlight: 33.1, dElev100: "23m" },
                { range: 2600, elevation: 831, timeOfFlight: 31.9, dElev100: "27m" },
                { range: 2700, elevation: 771, timeOfFlight: 30.5, dElev100: "0m" }
            ],
            // Ring 4 - MOD C-832C Illumination (ตามภาพ Ring 4 - Dispersion: 53m)
            4: [
                { range: 600, elevation: 1414, timeOfFlight: 50.3, dElev100: "7m" },
                { range: 700, elevation: 1400, timeOfFlight: 50.3, dElev100: "8m" },
                { range: 800, elevation: 1385, timeOfFlight: 50.2, dElev100: "7m" },
                { range: 900, elevation: 1370, timeOfFlight: 50.1, dElev100: "7m" },
                { range: 1000, elevation: 1355, timeOfFlight: 50.0, dElev100: "8m" },
                { range: 1100, elevation: 1340, timeOfFlight: 49.9, dElev100: "8m" },
                { range: 1200, elevation: 1324, timeOfFlight: 49.7, dElev100: "8m" },
                { range: 1300, elevation: 1308, timeOfFlight: 49.6, dElev100: "8m" },
                { range: 1400, elevation: 1292, timeOfFlight: 49.4, dElev100: "8m" },
                { range: 1500, elevation: 1276, timeOfFlight: 49.2, dElev100: "8m" },
                { range: 1600, elevation: 1260, timeOfFlight: 49.0, dElev100: "9m" },
                { range: 1700, elevation: 1243, timeOfFlight: 48.8, dElev100: "9m" },
                { range: 1800, elevation: 1225, timeOfFlight: 48.6, dElev100: "8m" },
                { range: 1900, elevation: 1208, timeOfFlight: 48.3, dElev100: "9m" },
                { range: 2000, elevation: 1190, timeOfFlight: 48.1, dElev100: "9m" },
                { range: 2100, elevation: 1172, timeOfFlight: 47.8, dElev100: "10m" },
                { range: 2200, elevation: 1153, timeOfFlight: 47.5, dElev100: "10m" },
                { range: 2300, elevation: 1134, timeOfFlight: 47.1, dElev100: "10m" },
                { range: 2400, elevation: 1114, timeOfFlight: 46.8, dElev100: "10m" },
                { range: 2500, elevation: 1093, timeOfFlight: 46.4, dElev100: "10m" },
                { range: 2600, elevation: 1072, timeOfFlight: 46.0, dElev100: "10m" },
                { range: 2700, elevation: 1051, timeOfFlight: 45.5, dElev100: "11m" },
                { range: 2800, elevation: 1028, timeOfFlight: 45.0, dElev100: "11m" },
                { range: 2900, elevation: 1005, timeOfFlight: 44.5, dElev100: "12m" },
                { range: 3000, elevation: 981, timeOfFlight: 44.0, dElev100: "13m" },
                { range: 3100, elevation: 955, timeOfFlight: 43.3, dElev100: "13m" },
                { range: 3200, elevation: 928, timeOfFlight: 42.7, dElev100: "13m" },
                { range: 3300, elevation: 900, timeOfFlight: 41.9, dElev100: "15m" },
                { range: 3400, elevation: 870, timeOfFlight: 41.1, dElev100: "16m" },
                { range: 3500, elevation: 838, timeOfFlight: 40.2, dElev100: "18m" },
                { range: 3600, elevation: 802, timeOfFlight: 39.2, dElev100: "19m" },
                { range: 3700, elevation: 763, timeOfFlight: 38.0, dElev100: "0m" }
            ]
        }
    },
    ...
    // dElev100 = Hw = |มุมยกที่ระยะ R+50 - มุมยกที่ระยะ R| หน่วยมิล (สูตรเดียวกับที่เกมใช้)
    m777: {
        M107: {
            1: [
                { range: 2900, elevation: 1266, timeOfFlight: 43.3, dElev100: "7m" },
                { range: 3000, elevation: 1252, timeOfFlight: 43.1, dElev100: "7m" },
                { range: 3100, elevation: 1237, timeOfFlight: 42.9, dElev100: "7m" },
                { range: 3200, elevation: 1223, timeOfFlight: 42.7, dElev100: "8m" },
                { range: 3300, elevation: 1208, timeOfFlight: 42.5, dElev100: "8m" },
                { range: 3400, elevation: 1192, timeOfFlight: 42.2, dElev100: "8m" },
                { range: 3500, elevation: 1176, timeOfFlight: 41.9, dElev100: "8m" },
                { range: 3600, elevation: 1160, timeOfFlight: 41.6, dElev100: "8m" },
                { range: 3700, elevation: 1143, timeOfFlight: 41.3, dElev100: "9m" },
                { range: 3800, elevation: 1125, timeOfFlight: 41.0, dElev100: "9m" },
                { range: 3900, elevation: 1106, timeOfFlight: 40.6, dElev100: "9m" },
                { range: 4000, elevation: 1087, timeOfFlight: 40.2, dElev100: "10m" },
                { range: 4100, elevation: 1066, timeOfFlight: 39.7, dElev100: "11m" },
                { range: 4200, elevation: 1044, timeOfFlight: 39.2, dElev100: "12m" },
                { range: 4300, elevation: 1020, timeOfFlight: 38.7, dElev100: "13m" },
                { range: 4400, elevation: 993, timeOfFlight: 38.0, dElev100: "14m" },
                { range: 4500, elevation: 963, timeOfFlight: 37.3, dElev100: "17m" },
                { range: 4600, elevation: 928, timeOfFlight: 36.4, dElev100: "22m" },
                { range: 4700, elevation: 881, timeOfFlight: 35.1, dElev100: "32m" }
            ],
            2: [
                { range: 4300, elevation: 1267, timeOfFlight: 54.0, dElev100: "5m" },
                { range: 4400, elevation: 1257, timeOfFlight: 53.8, dElev100: "4m" },
                { range: 4500, elevation: 1248, timeOfFlight: 53.6, dElev100: "5m" },
                { range: 4600, elevation: 1238, timeOfFlight: 53.4, dElev100: "5m" },
                { range: 4700, elevation: 1228, timeOfFlight: 53.2, dElev100: "5m" },
                { range: 4800, elevation: 1218, timeOfFlight: 53.1, dElev100: "5m" },
                { range: 4900, elevation: 1208, timeOfFlight: 52.8, dElev100: "5m" },
                { range: 5000, elevation: 1198, timeOfFlight: 52.6, dElev100: "5m" },
                { range: 5100, elevation: 1187, timeOfFlight: 52.4, dElev100: "5m" },
                { range: 5200, elevation: 1177, timeOfFlight: 52.2, dElev100: "6m" },
                { range: 5300, elevation: 1166, timeOfFlight: 51.9, dElev100: "6m" },
                { range: 5400, elevation: 1154, timeOfFlight: 51.7, dElev100: "6m" },
                { range: 5500, elevation: 1143, timeOfFlight: 51.4, dElev100: "6m" },
                { range: 5600, elevation: 1131, timeOfFlight: 51.1, dElev100: "6m" },
                { range: 5700, elevation: 1118, timeOfFlight: 50.8, dElev100: "6m" },
                { range: 5800, elevation: 1106, timeOfFlight: 50.5, dElev100: "7m" },
                { range: 5900, elevation: 1093, timeOfFlight: 50.2, dElev100: "7m" },
                { range: 6000, elevation: 1079, timeOfFlight: 49.8, dElev100: "7m" },
                { range: 6100, elevation: 1065, timeOfFlight: 49.4, dElev100: "7m" },
                { range: 6200, elevation: 1050, timeOfFlight: 49.0, dElev100: "8m" },
                { range: 6300, elevation: 1035, timeOfFlight: 48.6, dElev100: "8m" },
                { range: 6400, elevation: 1018, timeOfFlight: 48.1, dElev100: "8m" },
                { range: 6500, elevation: 1001, timeOfFlight: 47.6, dElev100: "9m" },
                { range: 6600, elevation: 982, timeOfFlight: 47.1, dElev100: "10m" },
                { range: 6700, elevation: 962, timeOfFlight: 46.4, dElev100: "11m" },
                { range: 6800, elevation: 940, timeOfFlight: 45.7, dElev100: "13m" },
                { range: 6900, elevation: 914, timeOfFlight: 44.9, dElev100: "14m" },
                { range: 7000, elevation: 884, timeOfFlight: 43.9, dElev100: "18m" },
                { range: 7100, elevation: 844, timeOfFlight: 42.4, dElev100: "30m" }
            ],
            3: [
                { range: 6800, elevation: 1271, timeOfFlight: 70.9, dElev100: "3m" },
                { range: 7000, elevation: 1259, timeOfFlight: 70.6, dElev100: "3m" },
                { range: 7200, elevation: 1247, timeOfFlight: 70.3, dElev100: "3m" },
                { range: 7400, elevation: 1235, timeOfFlight: 70.0, dElev100: "3m" },
                { range: 7600, elevation: 1222, timeOfFlight: 69.7, dElev100: "3m" },
                { range: 7800, elevation: 1210, timeOfFlight: 69.4, dElev100: "4m" },
                { range: 8000, elevation: 1197, timeOfFlight: 69.1, dElev100: "4m" },
                { range: 8200, elevation: 1183, timeOfFlight: 68.7, dElev100: "3m" },
                { range: 8400, elevation: 1169, timeOfFlight: 68.3, dElev100: "3m" },
                { range: 8600, elevation: 1155, timeOfFlight: 67.9, dElev100: "3m" },
                { range: 8800, elevation: 1141, timeOfFlight: 67.5, dElev100: "4m" },
                { range: 9000, elevation: 1126, timeOfFlight: 67.0, dElev100: "4m" },
                { range: 9200, elevation: 1110, timeOfFlight: 66.5, dElev100: "4m" },
                { range: 9400, elevation: 1094, timeOfFlight: 66.0, dElev100: "4m" },
                { range: 9600, elevation: 1077, timeOfFlight: 65.4, dElev100: "4m" },
                { range: 9800, elevation: 1060, timeOfFlight: 64.8, dElev100: "5m" },
                { range: 10000, elevation: 1041, timeOfFlight: 64.1, dElev100: "5m" },
                { range: 10200, elevation: 1021, timeOfFlight: 63.4, dElev100: "5m" },
                { range: 10400, elevation: 1000, timeOfFlight: 62.6, dElev100: "5m" },
                { range: 10600, elevation: 978, timeOfFlight: 61.8, dElev100: "6m" },
                { range: 10800, elevation: 953, timeOfFlight: 60.8, dElev100: "7m" },
                { range: 11000, elevation: 925, timeOfFlight: 59.6, dElev100: "8m" },
                { range: 11200, elevation: 893, timeOfFlight: 58.2, dElev100: "9m" },
                { range: 11400, elevation: 853, timeOfFlight: 56.5, dElev100: "12m" }
            ],
            4: [
                { range: 8800, elevation: 1274, timeOfFlight: 83.7, dElev100: "2m" },
                { range: 9000, elevation: 1265, timeOfFlight: 83.5, dElev100: "2m" },
                { range: 9200, elevation: 1256, timeOfFlight: 83.3, dElev100: "2m" },
                { range: 9400, elevation: 1247, timeOfFlight: 83.0, dElev100: "2m" },
                { range: 9600, elevation: 1238, timeOfFlight: 82.7, dElev100: "3m" },
                { range: 9800, elevation: 1228, timeOfFlight: 82.5, dElev100: "2m" },
                { range: 10000, elevation: 1218, timeOfFlight: 82.2, dElev100: "2m" },
                { range: 10200, elevation: 1209, timeOfFlight: 81.9, dElev100: "3m" },
                { range: 10400, elevation: 1199, timeOfFlight: 81.6, dElev100: "3m" },
                { range: 10600, elevation: 1189, timeOfFlight: 81.3, dElev100: "3m" },
                { range: 10800, elevation: 1178, timeOfFlight: 80.9, dElev100: "2m" },
                { range: 11000, elevation: 1168, timeOfFlight: 80.6, dElev100: "3m" },
                { range: 11200, elevation: 1157, timeOfFlight: 80.2, dElev100: "3m" },
                { range: 11400, elevation: 1146, timeOfFlight: 79.8, dElev100: "3m" },
                { range: 11600, elevation: 1135, timeOfFlight: 79.4, dElev100: "3m" },
                { range: 11800, elevation: 1123, timeOfFlight: 79.0, dElev100: "3m" },
                { range: 12000, elevation: 1111, timeOfFlight: 78.6, dElev100: "3m" },
                { range: 12200, elevation: 1099, timeOfFlight: 78.1, dElev100: "3m" },
                { range: 12400, elevation: 1087, timeOfFlight: 77.6, dElev100: "4m" },
                { range: 12600, elevation: 1074, timeOfFlight: 77.1, dElev100: "4m" },
                { range: 12800, elevation: 1060, timeOfFlight: 76.6, dElev100: "3m" },
                { range: 13000, elevation: 1047, timeOfFlight: 76.0, dElev100: "4m" },
                { range: 13200, elevation: 1032, timeOfFlight: 75.4, dElev100: "4m" },
                { range: 13400, elevation: 1017, timeOfFlight: 74.8, dElev100: "4m" },
                { range: 13600, elevation: 1002, timeOfFlight: 74.1, dElev100: "5m" },
                { range: 13800, elevation: 985, timeOfFlight: 73.4, dElev100: "4m" },
                { range: 14000, elevation: 968, timeOfFlight: 72.6, dElev100: "5m" },
                { range: 14200, elevation: 949, timeOfFlight: 71.7, dElev100: "5m" },
                { range: 14400, elevation: 929, timeOfFlight: 70.8, dElev100: "5m" },
                { range: 14600, elevation: 907, timeOfFlight: 69.7, dElev100: "6m" },
                { range: 14800, elevation: 883, timeOfFlight: 68.5, dElev100: "7m" },
                { range: 15000, elevation: 855, timeOfFlight: 67.0, dElev100: "8m" },
                { range: 15200, elevation: 820, timeOfFlight: 65.2, dElev100: "10m" }
            ],
            5: [
                { range: 10750, elevation: 1269, timeOfFlight: 94.6, dElev100: "2m" },
                { range: 11000, elevation: 1260, timeOfFlight: 94.3, dElev100: "2m" },
                { range: 11250, elevation: 1250, timeOfFlight: 94.0, dElev100: "2m" },
                { range: 11500, elevation: 1241, timeOfFlight: 93.7, dElev100: "2m" },
                { range: 11750, elevation: 1231, timeOfFlight: 93.4, dElev100: "2m" },
                { range: 12000, elevation: 1221, timeOfFlight: 93.0, dElev100: "2m" },
                { range: 12250, elevation: 1211, timeOfFlight: 92.7, dElev100: "2m" },
                { range: 12500, elevation: 1201, timeOfFlight: 92.3, dElev100: "2m" },
                { range: 12750, elevation: 1190, timeOfFlight: 92.0, dElev100: "2m" },
                { range: 13000, elevation: 1180, timeOfFlight: 91.6, dElev100: "2m" },
                { range: 13250, elevation: 1169, timeOfFlight: 91.2, dElev100: "2m" },
                { range: 13500, elevation: 1158, timeOfFlight: 90.8, dElev100: "2m" },
                { range: 13750, elevation: 1146, timeOfFlight: 90.3, dElev100: "2m" },
                { range: 14000, elevation: 1135, timeOfFlight: 89.9, dElev100: "2m" },
                { range: 14250, elevation: 1123, timeOfFlight: 89.4, dElev100: "2m" },
                { range: 14500, elevation: 1111, timeOfFlight: 88.9, dElev100: "3m" },
                { range: 14750, elevation: 1099, timeOfFlight: 88.4, dElev100: "3m" },
                { range: 15000, elevation: 1086, timeOfFlight: 87.8, dElev100: "3m" },
                { range: 15250, elevation: 1073, timeOfFlight: 87.3, dElev100: "3m" },
                { range: 15500, elevation: 1059, timeOfFlight: 86.7, dElev100: "3m" },
                { range: 15750, elevation: 1045, timeOfFlight: 86.0, dElev100: "3m" },
                { range: 16000, elevation: 1030, timeOfFlight: 85.3, dElev100: "3m" },
                { range: 16250, elevation: 1015, timeOfFlight: 84.6, dElev100: "3m" },
                { range: 16500, elevation: 999, timeOfFlight: 83.8, dElev100: "3m" },
                { range: 16750, elevation: 983, timeOfFlight: 83.0, dElev100: "4m" },
                { range: 17000, elevation: 965, timeOfFlight: 82.1, dElev100: "4m" },
                { range: 17250, elevation: 946, timeOfFlight: 81.2, dElev100: "3m" },
                { range: 17500, elevation: 927, timeOfFlight: 80.1, dElev100: "5m" },
                { range: 17750, elevation: 905, timeOfFlight: 78.9, dElev100: "4m" },
                { range: 18000, elevation: 881, timeOfFlight: 77.6, dElev100: "5m" },
                { range: 18250, elevation: 855, timeOfFlight: 76.1, dElev100: "6m" },
                { range: 18500, elevation: 824, timeOfFlight: 74.2, dElev100: "7m" }
            ]
        }
    }
};
...
// AVERAGE DISPERSION (เมตร) — ตัวเลขเดียวกับที่เกมพิมพ์ไว้มุมบนขวาของใบตารางยิง
//
// ที่มา: `m_fStandardDispersion` ของแต่ละหน้าใน SCR_BallisticTableComponent
//   เกมต้นฉบับ  -> data007.pak : Prefabs/Items/Equipment/BallisticTable/BallisticTable_US_base.et
//                                 และ BallisticTable_USSR_base.et
//   Adult Mortars -> data.pak  : .../BalisticTable_US.et และ BalisticTable_USSR.et
//   M777          -> data.pak  : .../BallisticTable_US_Artillery.et
// เกมวาดมันที่ SCR_VisualisedBallisticConfig.c:164-170 ด้วยฟอร์แมต "%1 m" ใต้ป้าย "AVERAGE DISPERSION"
//
// เป็นค่าเดียวต่อ (กระสุน x ประจุ) ไม่ใช่ค่ารายระยะ — เกมไม่มีข้อมูลระดับนั้น
// จับคู่หน้าตาราง -> ประจุ ด้วย m_fProjectileInitSpeedCoef ซึ่งตรงกับ m_aChargeRingConfig ของกระสุนทุกตัว
const DISPERSION_DATA = {
    original: {
        M821: { 0: 10, 1: 23, 2: 39, 3: 54, 4: 69 },
        M819: { 1: 18, 2: 33, 3: 46, 4: 57 },
        M853A1: { 1: 17, 2: 34, 3: 48, 4: 59 },
        '0-832Ay': { 0: 13, 1: 21, 2: 33, 3: 44, 4: 55 },
        'A-832AY': { 0: 12, 1: 19, 2: 30, 3: 40 },
        'C-832C': { 1: 15, 2: 28, 3: 39, 4: 53 }
    },
    mod: {
        M821: { 0: 5, 1: 21, 2: 31, 3: 34, 4: 43 },
        M819: { 1: 9, 2: 12, 3: 16, 4: 20 },
        M853A1: { 1: 7, 2: 11, 3: 17, 4: 20 },
        '0-832Ay': { 0: 6, 1: 14, 2: 17, 3: 26, 4: 34 },
        'A-832AY': { 0: 3, 1: 9, 2: 14, 3: 20 },
        'C-832C': { 1: 5, 2: 8, 3: 13, 4: 19 }
    },
    // M777 มีสองชุด: วิถีโค้งสูง (High angle) กับวิถีราบ (Low angle)
    // เว็บคำนวณเฉพาะวิถีโค้งสูง จึงใช้ชุด High angle
    m777: {
        M107: { 1: 21.2, 2: 45, 3: 50, 4: 55, 5: 73 }
    }
};
...
M777_SHELL_ALIASES.forEach(name => {
    ...
    DISPERSION_DATA.m777[name] = DISPERSION_DATA.m777.M107;
});
...
class MortarCalculator {
    ...
    // AVERAGE DISPERSION ของประจุที่กำลังใช้ (เมตร) — ค่าเดียวกับที่เกมพิมพ์บนใบตาราง
    // เป็นค่าคงที่ต่อประจุ ไม่แปรตามระยะ (เกมไม่มีข้อมูลรายระยะ)
    getDispersionRadius(ring) {
        const set = DISPERSION_DATA[this.currentMortarType];
        const d = set && set[this.currentShell];
        if (!d) return null;
        const rg = (ring === undefined || ring === null) ? this.currentCharge : ring;
        const v = d[rg];
        return (typeof v === 'number') ? v : null;
    }

    // "(รัศมี 43 m)" ต่อท้ายเลขประจุ — ว่างเปล่าถ้ากระสุนนั้นไม่มีข้อมูล
    // ค่าที่ประกอบเป็นตัวเลขของเราเองกับข้อความแปล จึงใส่เป็น HTML ได้อย่างปลอดภัย
    chargeDispersionSuffix(ring) {
        const r = this.getDispersionRadius(ring);
        if (r === null) return '';
        const v = Number.isInteger(r) ? r : r.toFixed(1);
        return ` <span class="charge-dispersion">(${T('radiusLabel')} ${v} m)</span>`;
    }

    formatDispersion(ring) {
        const r = this.getDispersionRadius(ring);
        if (r === null) return '—';
    ...
    // คำอธิบายหัวคอลัมน์ — คอลัมน์ D ELEV / 100M เคยถูกตั้งชื่อผิดว่า "การกระจาย"
    // จึงใส่ tooltip กำกับไว้ให้ชัดว่าคนละเรื่องกัน
    updateTableHeaderTitles() {
        const thDelev = document.getElementById('th-delev');
        const thDisp = document.getElementById('th-dispersion');
        if (thDelev) thDelev.title = T('dElevPer100mHint');
        if (thDisp) thDisp.title = T('dispersionRadiusHint');
    }
    ...
    loadBallisticData() {
        ...
        this.updateTableHeaderTitles();

        if (!data || data.length === 0) {
            this.ballisticTbody.innerHTML = '<tr><td colspan="5">No data available</td></tr>';
            return;
        }
    ...
    selectOptimalCharge(targetDistance) {
        ...
        Object.keys(shellData).forEach(charge => {
            ...
            // Filter out invalid data (dElev100 = "0m")
            const validData = chargeData.filter(item => item.dElev100 !== "0m");
    ...
    findBallisticData(distance) {
        ...
        // กรองข้อมูลที่มี dElev100 = "0m" ออก เพราะไม่ควรใช้ในการคำนวณ
        const validData = data.filter(item => item.dElev100 !== "0m");
        ...
        if (lower && upper && lower.range !== upper.range) {
            ...
            return {
                ...
                dElev100: lower.dElev100, // ใช้ค่า dElev100 ของจุดล่าง
    ...
    // ใช้ค่า dElev100 จาก BALLISTIC_DATA เป็นฐานในการคำนวณ
    calculateElevationCorrection(heightDiff, ballisticData) {
        // ตรวจสอบว่ามีข้อมูล ballistic data หรือไม่
        if (!ballisticData || !ballisticData.dElev100) {
            return 0;
        }

        // แปลงค่า dElev100 จากรูปแบบ "xxm" เป็นตัวเลข
        const dElev100Value = parseFloat(ballisticData.dElev100.replace('m', ''));

        // ตรวจสอบว่าค่า dElev100 เป็น 0 หรือไม่ถูกต้องหรือไม่
        if (isNaN(dElev100Value) || dElev100Value <= 0) {
            return 0;
        }
        ...
        const correction = (dElev100Value / 100) * Math.abs(heightDiff);
    ...
    // silent = วาดใหม่เฉย ๆ (ใช้ตอนสลับภาษา) ไม่ต้องเด้งเตือนถ้าพิกัดยังกรอกไม่ครบ
    calculateAvailableRings(silent) {
        if (!this.validateInputs()) {
            if (!silent) this.showError(T('fillAllFieldsMessage'));
            return;
        }
    ...
    displayRingResults(rings, targetDistance, wasAdjusted) {
        this._lastRingArgs = { rings, targetDistance, wasAdjusted };
        const texts = LANGUAGE_DATA[currentLanguage];
    ...
    isRingUsable(ring, distance, heightDiff, azimuthDeg) {
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (adjD >= a.range && adjD <= b.range) {
                ...
                row = { range: adjD, elevation: a.elevation + (b.elevation - a.elevation) * t, dElev100: a.dElev100 };
                break;
            }
        }
    ...
    calculate() {
        ...
        let outOfTable = null;
        const rangeRows = (this.getCurrentBallisticData() || []).filter(r => r.dElev100 !== '0m');
        if (rangeRows.length) {
        ...
        // ใช้สูตร: (dElev100 ÷ 100) × ความต่างระดับความสูงที่ปรับแล้ว
        ...
        this.displayResults({
            ...
            dElev100: ballisticData.dElev100,
    ...
    displayResults(results) {
        ...
        // ต่อท้ายด้วยรัศมีการกระจายของประจุที่ใช้ (ค่าเดียวกับมุมบนขวาของใบตารางในเกม)
        if (adv) {
            this.chargeEl.innerHTML = `${shown.charge}${this.chargeDispersionSuffix(shown.charge)}`;
        } else {
            this.chargeEl.textContent = T('noSolutionCharge');
        }
    ...
    updateFixedFiringSolution(results) {
        ...
        const fsCharge = document.getElementById('fs-charge');
        if (ok) {
            fsCharge.innerHTML = `${shown.charge}${this.chargeDispersionSuffix(shown.charge)}`;
        } else {
            fsCharge.textContent = T('noSolutionCharge');
        }
